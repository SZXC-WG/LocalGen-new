...
//			movewindow((nScreenWidth - 1600 * mapDataStore.mapSize) / 2, (nScreenHeight - 900 * mapDataStore.mapSize) / 2, true);
	...
	void doMapSelect() {
		...
		for (int i = 1; i <= mapNum; i++) {
			...
			mapbut[i].setfonthw(18 * mapDataStore.mapSizeY, 0);
			...
			mapbut[i].addtext("GeneralCount: " + to_string(maps[i].generalcnt));
			mapbut[i].addtext("SwampCount: " + to_string(maps[i].swampcnt));
			mapbut[i].addtext("MountainCount: " + to_string(maps[i].mountaincnt));
			mapbut[i].addtext("CityCount: " + to_string(maps[i].citycnt));
