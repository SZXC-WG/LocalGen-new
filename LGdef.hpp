...
constexpr char NUM_s[20] = {
    0,   'H', 'K', 'W', 'L', 'M',
    'Q', 'I', 'G', 'B', 'N', 'T'};  // number suffixes; 10^2<->10^12
constexpr int LEN_ZIP = 100005, CHAR_AD = 48, LEN_MOVE = 30005,
              replaySorter = 2000;
...
/* block type: 0->plain, 1->swamp, 2->mountain, 3->general, 4->city, 5->desert,
 * 6->lookout, 7->observatory */
...
playerS playerInfo[64] = {
    ...
    {L"LightBlue", 0xff2792FF},  {L"Green", 0xff008000},
    {L"Teal", 0xff008080},       {L"Orange", 0xffFF7010},
    {L"Pink", 0xffF032E6},       {L"Purple", 0xff800080},
    {L"Maroon", 0xff9B0101},     {L"Yellow", 0xffB3AC32},
    {L"Brown", 0xff9A5E24},      {L"Blue", 0xff1031FF},
    {L"PurpleBlue", 0xff594CA5}, {L"YellowGreen", 0xff85A91C},
    {L"LightRed", 0xffF87375},   {L"LightPurple", 0xffB47FCA},
    {L"LightBrown", 0xffB49971},
};
...
char strdeGameZip[LEN_ZIP << 2];    // storing the to-be-dezipped zipped game
                                    // (deprecated)
...
void createRandomMap(int crtH,
                     int crtW);  // generate random map (special) with H and W
void createStandardMap(
    int crtH, int crtW);  // generate standard map (special) with H and W
void createMazeMap(
    int crtH, int crtW, int ringTYPE,
    int routeTYPE);  // generate labyrinth map (special) with H and W
...
/* LocalGen Modifiers: */
bool DeepSwamp = false;      // [todo) swamp drain speed increase as the
                             // dist to land increase.
bool SuburbPlain = false;    // [todo) plain inc speed increase as the
                             // dist to cities decrease.
bool NeutralResist = false;  // neutral troops increase half to one
                             // third the speed of players.
}  // namespace modifier
}  // namespace game
...
bool FullScreen(HWND hwnd, int fullscreenWidth = GetSystemMetrics(SM_CXSCREEN),
                int fullscreenHeight = GetSystemMetrics(SM_CYSCREEN),
                int colourBits = 32, int refreshRate = 60);
...
inline __attribute__((always_inline)) double zoomX(double X) {
    return X * windowData.zoomX;
}
inline __attribute__((always_inline)) double zoomY(double Y) {
    return Y * windowData.zoomY;
}
void WelcomePage();
void settingsPage();
void localOptions();
