...
namespace LGlocal {
	int GAME() {
		...
		for(int i = 2; i <= LGgame::playerCnt; ++i) {
			...
			if(600 <= LGgame::robotId[i] && LGgame::robotId[i] < 700) gcBot::initBot(i);
		}
		...
		for(; is_run();) {
						...
						if(!moved && now - prsttm < 200ms) {
							if(msg.x >= LGGraphics::windowData.maplocX &&
							   ...
							   msg.y <= LGGraphics::windowData.maplocY + int(blockHeight + LGGraphics::zoomY(2)) * mapH) {
								...
								moveS mv{
									1, false, LGgame::playerFocus[1], coordS{ lin, col }
								};
			...
			if(1) {
				...
				if(lackTurn <= 0 || gamePaused) {
					...
					rspeedlen = textwidth((L"Real Speed: " + to_wstring(LGgame::curTurn * 1.0L / (timePassed.count() / 1'000'000'000.0L))).c_str());
					setfillcolor(RED);
					setfillcolor(GREEN);
