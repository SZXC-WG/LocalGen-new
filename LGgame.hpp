...
inline bool LGgame::unpassable(int t) { return t == 2 || t == 6 || t == 7; }
...
int LGgame::analyzeMove(int id, int mv, coordS& coo) {
    switch (mv) {
        ...
        case 1 ... 4: {
            ...
            if (newCoo.x < 1 || newCoo.x > mapH || newCoo.y < 1 ||
                newCoo.y > mapW || unpassable(gameMap[newCoo.x][newCoo.y].type))
                return 1;
            moveS insMv{
                id,
                true,
...
int LGgame::checkMove(moveS mv) {
    ...
    if (mv.from.x < 1 || mv.from.x > mapH || mv.from.y < 1 || mv.from.y > mapW)
        return 2;
    if (mv.to.x < 1 || mv.to.x > mapH || mv.to.y < 1 || mv.to.y > mapW)
        return 2;
    ...
    if (abs(mv.to.x - mv.from.x) + abs(mv.to.y - mv.from.y) > 1 && mv.takeArmy)
        return 3;  // focus change
    return 0;
}
        ...
        if (gameMap[cur.to.x][cur.to.y].player == cur.id) {
            ...
            if (gameMap[cur.to.x][cur.to.y].army < 0) {
                ...
                if (gameMap[cur.to.x][cur.to.y].type == 3) {
                    /* general */
                    capture(cur.id, p);
                    // gameMap[cur.to.x][cur.to.y].type = 4; // move to function
                    // capture(int,int)
                    /* for(auto& mv : LGgame::inlineMove)
                        if(mv.id == p)
                            mv.id = cur.id; */
                    // useless and cause bugs
                }
            }
        }
        LGgame::playerCoo[cur.id] = cur.to;
...
void LGgame::initGenerals(coordS coos[]) {
    ...
    for (int i = 1; i <= mapH; ++i)
        for (int j = 1; j <= mapW; ++j)
            if (gameMap[i][j].type == 3) gens.push_back(coordS{i, j});
    ...
    sort(gens.begin(), gens.end(),
         [](coordS a, coordS b) { return a.x == b.x ? a.y < b.y : a.x < b.x; });
    std::shuffle(
        gens.begin(), gens.end(),
        std::mt19937(
            std::chrono::system_clock::now().time_since_epoch().count()));
    ...
    if (LGset::gameMode == 1) {
        for (int i = 1; i <= LGgame::playerCnt; ++i)
            gmt(LGgame::genCoo[i]) = 4, gma(LGgame::genCoo[i]) = 1;
    } else if (LGset::gameMode == 2) {
        for (int i = 1; i <= LGgame::playerCnt; ++i)
            gmt(LGgame::genCoo[i]) = 0, gma(LGgame::genCoo[i]) = 1;
    }
}
            ...
            if (gameMap[i][j].player != 0) {
                ...
                switch (gameMap[i][j].type) {
                    case 0: {
                        /* plain */
                        if (LGgame::curTurn %
                                (2 * LGset::plainRate[LGset::gameMode]) ==
                            0)
                            ++gameMap[i][j].army;
                        break;
                    }
...
void LGgame::statistics() {
    for (int i = 1; i <= LGgame::playerCnt; ++i)
        gameStats[i].push_back(turnStatS());
    for (int i = 1; i <= LGgame::playerCnt; ++i) gameStats[i].back().id = i;
    for (int i = 1; i <= LGgame::playerCnt; ++i)
        gameStats[i].back().focus = LGgame::playerFocus[i];
    for (int i = 1; i <= LGgame::playerCnt; ++i)
        gameStats[i].back().coo = LGgame::playerCoo[i];
    for (int i = 1; i <= mapH; ++i) {
        for (int j = 1; j <= mapW; ++j) {
            if (gmp(i, j) == 0) continue;
            gameStats[gmp(i, j)].back().army += gma(i, j);
            if (gmt(i, j) == 0)
                gameStats[gmp(i, j)].back().plain += 1;
            else if (gmt(i, j) == 1)
                gameStats[gmp(i, j)].back().swamp += 1;
            else if (gmt(i, j) == 2)
                gameStats[gmp(i, j)].back().mount += 1;
            else if (gmt(i, j) == 3)
                gameStats[gmp(i, j)].back().city += 1;
            else if (gmt(i, j) == 4)
                gameStats[gmp(i, j)].back().city += 1;
            else if (gmt(i, j) == 5)
                gameStats[gmp(i, j)].back().desert += 1;
        }
    }
}
...
void LGgame::ranklist(bool print) {
    ...
    static int nlen = -10, alen = -10, plen = -10, clen = -10, tlen = -10,
               inclen = -10, aihlen = -10, botlen = -10;
    setfillcolor(LGGraphics::bgColor);
    // bar(1600 * LGGraphics::windowData.mapSizeX - nlen - alen - plen - clen -
    // tlen - aihlen - botlen - 35, 20 * LGGraphics::windowData.mapSizeY,
    //     1600 * LGGraphics::windowData.mapSizeX, (LGgame::playerCnt + 2) * 20
    //     * LGGraphics::windowData.mapSizeY + 5 + 5);
    struct node {
        int id;
        long long army;
    ...
    for (int i = 1; i <= LGgame::playerCnt; ++i) {
        ...
        rklst[i].inc = LGgame::gameStats[i].back().plain /
                           LGset::plainRate[LGset::gameMode] +
                       LGgame::gameStats[i].back().city -
                       LGgame::gameStats[i].back().swamp;
        rklst[i].tot = LGgame::gameStats[i].back().gtot();
    }
    if (!print) return;
    std::sort(rklst + 1, rklst + LGgame::playerCnt + 1,
              [](node a, node b) { return a.army > b.army; });
    ...
    for (int i = 1; i <= LGgame::playerCnt; ++i) {
        ...
        if (!LGset::modifier::SilentWar)
            inclen = max(inclen, textwidth(to_string(rklst[i].inc).c_str()));
        if (!LGset::modifier::SilentWar && printAIH)
            aihlen =
                max(aihlen, textwidth(to_string(rklst[i].armyInHand).c_str()));
        if (printBot)
            botlen = max(
                botlen,
                textwidth(
                    botName[LGgame::robotId[rklst[i].id] / 100 + 1].c_str()));
    }
    ...
    for (int i = 1; i <= LGgame::playerCnt; i++) {
        ...
        if (!LGset::modifier::SilentWar && printAIH)
            rectangle(s7, prhei * (i + 1), s8, prhei * (i + 2));
        if (printBot) rectangle(s8, prhei * (i + 1), ed, prhei * (i + 2));
        xyprintf((s1 + s2) / 2, prhei * (i + 1),
                 playerInfo[rklst[i].id].name.c_str());
        if (!LGset::modifier::SilentWar)
            xyprintf((s2 + s3) / 2, prhei * (i + 1),
                     to_string(rklst[i].army).c_str());
        if (!LGset::modifier::SilentWar)
            xyprintf((s3 + s4) / 2, prhei * (i + 1),
                     to_string(rklst[i].plain).c_str());
        if (!LGset::modifier::SilentWar)
            xyprintf((s4 + s5) / 2, prhei * (i + 1),
                     to_string(rklst[i].city).c_str());
        if (!LGset::modifier::SilentWar)
            xyprintf((s5 + s6) / 2, prhei * (i + 1),
                     to_string(rklst[i].tot).c_str());
        if (!LGset::modifier::SilentWar)
            xyprintf((s6 + s7) / 2, prhei * (i + 1),
                     to_string(rklst[i].inc).c_str());
        if (!LGset::modifier::SilentWar && printAIH)
            xyprintf((s7 + s8) / 2, prhei * (i + 1),
                     to_string(rklst[i].armyInHand).c_str());
        if (printBot)
            xyprintf((s8 + ed) / 2, prhei * (i + 1),
                     botName[LGgame::robotId[rklst[i].id] / 100 + 1].c_str());
    }
}
void LGgame::printAnalysis() {
#define _log(b, x) \
    (((x) != -1) ? (((x) == 0) ? (0) : (log(x) / log(b) + 1)) : -1)
    ...
    static double landX[64], landY[64], armyX[64], armyY[64], cityX[64],
        cityY[64];
    static PIMAGE landI = newimage(600 * LGGraphics::windowData.zoomX,
                                   200 * LGGraphics::windowData.zoomY),
                  armyI = newimage(600 * LGGraphics::windowData.zoomX,
                                   200 * LGGraphics::windowData.zoomY),
                  cityI = newimage(600 * LGGraphics::windowData.zoomX,
                                   200 * LGGraphics::windowData.zoomY);
    ...
    const int graphRDX = 600 * LGGraphics::windowData.zoomX,
              graphRDY = 200 * LGGraphics::windowData.zoomY;
    ...
    static const auto ulArmy = [&]() -> long long {
        long long upperLimit = 0;
        for (int i = LGgame::playerCnt; i >= 1; --i)
            for (auto x : gameStats[i]) upperLimit = max(upperLimit, x.army);
        return upperLimit;
    };
        ...
        for (int i = LGgame::playerCnt; i >= 1; --i)
            for (auto x : gameStats[i])
                upperLimit = max(
                    upperLimit,
                    (long long)(x.city +
                                x.plain / LGset::plainRate[LGset::gameMode]));
        return upperLimit;
    };
    static const auto redrawLand = [&]() -> void {
        cleardevice(landI);
        // settextjustify(LEFT_TEXT,CENTER_TEXT);
        // xyprintf(0,800 * LGGraphics::windowData.zoomY,"upperLimit:
        // %lld",upperLimit);
        ...
        for (int i = LGgame::playerCnt; i >= 1; --i) {
            ...
            for (int j = 0; j <= curTurn; ++j) {
                ...
                double nx =
                    _log(lB, ct) * (graphRDX - 0) * 1.0 / _log(lB, XTurn);
                double ny =
                    _log(lB, a) * (graphRDY - 0) * 1.0 / _log(lB, YMaxLand);
                if (nx - cx <= 1 && !(ny == 0 && cy != 0)) continue;
                line(0 + cx, graphRDY - cy, 0 + nx, graphRDY - ny, landI);
                cx = nx, cy = ny, ca = a;
            }
            landX[i] = cx, landY[i] = cy;
            // xyprintf(0,(800-15*(LGgame::playerCnt-i+1)) *
            // LGGraphics::windowData.zoomY,"%ls - army:%lld cx:%.1f cy:%.1f
            // ca:%.1f",playerInfo[i].name.c_str(),historyArmy[i].back().second,cx,cy,ca);
        }
    };
        ...
        for (int i = LGgame::playerCnt; i >= 1; --i) {
            ...
            for (int j = 0; j <= curTurn; ++j) {
                ...
                double nx =
                    _log(lB, ct) * (graphRDX - 0) * 1.0 / _log(lB, XTurn);
                double ny =
                    _log(lB, a) * (graphRDY - 0) * 1.0 / _log(lB, YMaxArmy);
                if (nx - cx <= 1 && !(ny == 0 && cy != 0)) continue;
    ...
    static auto redrawCity = [&]() -> void {
        ...
        for (int i = LGgame::playerCnt; i >= 1; --i) {
            ...
            for (int j = 0; j <= curTurn; ++j) {
                int ct = j;
                long long a;
                a = gameStats[i][j].city +
                    gameStats[i][j].plain / LGset::plainRate[LGset::gameMode];
                double nx =
                    _log(lB, ct) * (graphRDX - 0) * 1.0 / _log(lB, XTurn);
                double ny =
                    _log(lB, a) * (graphRDY - 0) * 1.0 / _log(lB, YMaxCity);
                if (nx - cx <= 1 && !(ny == 0 && cy != 0)) continue;
    ...
    if (flagLand)
        ...
        for (int i = LGgame::playerCnt; i >= 1; --i) {
            ...
            line(0 + landX[i], graphRDY - landY[i], 0 + cx, graphRDY - cy,
                 landI);
            landX[i] = cx, landY[i] = cy;
        }
    }
    if (flagArmy)
        ...
        for (int i = LGgame::playerCnt; i >= 1; --i) {
            ...
            line(0 + armyX[i], graphRDY - armyY[i], 0 + cx, graphRDY - cy,
                 armyI);
            armyX[i] = cx, armyY[i] = cy;
        }
    }
    if (flagcity)
        ...
        for (int i = LGgame::playerCnt; i >= 1; --i) {
            ...
            int ct = curTurn;
            long long a;
            a = gameStats[i].back().city +
                gameStats[i].back().plain / LGset::plainRate[LGset::gameMode];
            cx = _log(lB, ct) * (graphRDX - 0) * 1.0 / _log(lB, XTurn);
            cy = _log(lB, a) * (graphRDY - 0) * 1.0 / _log(lB, YMaxCity);
            if (cx - cityX[i] <= 1 && !(cy == 0 && cityY[i] != 0)) continue;
            line(0 + cityX[i], graphRDY - cityY[i], 0 + cx, graphRDY - cy,
                 cityI);
            cityX[i] = cx, cityY[i] = cy;
        }
    }
    putimage(1000 * LGGraphics::windowData.zoomX,
             300 * LGGraphics::windowData.zoomY, landI);
    putimage(1000 * LGGraphics::windowData.zoomX,
             500 * LGGraphics::windowData.zoomY, armyI);
    putimage(1000 * LGGraphics::windowData.zoomX,
             700 * LGGraphics::windowData.zoomY, cityI);
#undef _log
}
...
int GAME() {
    ...
    LGGraphics::inputMapData(std::min(900 / mapH, 900 / mapW),
    ...
    for (int i = 2; i <= LGgame::playerCnt; ++i) {
        ...
        // LGgame::robotId[i] = mtrd() % 200 + 300;
        // LGgame::robotId[i] = mtrd() % 100 + 300;
        if (300 <= LGgame::robotId[i] && LGgame::robotId[i] < 400)
            zlyBot::initBot(i);
        if (400 <= LGgame::robotId[i] && LGgame::robotId[i] < 500)
            szlyBot::initBot(i);
        if (500 <= LGgame::robotId[i] && LGgame::robotId[i] < 600)
            zlyBot_v2::initBot(i);
    ...
    for (int i = 1; i <= LGgame::playerCnt; ++i)
        LGgame::playerFocus[i] = LGgame::playerCoo[i] = LGgame::genCoo[i];
    ...
    flushmouse();
    int midact = 0;
    LGGraphics::windowData.maplocX = -(LGgame::genCoo[1].y) * blockWidth +
                                     800 * LGGraphics::windowData.zoomX;
    LGGraphics::windowData.maplocY = -(LGgame::genCoo[1].x) * blockHeight +
                                     450 * LGGraphics::windowData.zoomY;
    int smsx = 0, smsy = 0;
    bool moved = false;
    ...
    for (; is_run();) {
        while (mousemsg()) {
            ...
            if (msg.is_wheel()) {
                ...
                int blx =
                    (msg.y - LGGraphics::windowData.maplocY) / blockHeight;
                        ...
                        if (msg.x >= LGGraphics::windowData.maplocX &&
                            ...
                            msg.x <=
                                LGGraphics::windowData.maplocX +
                                    int(blockWidth + LGGraphics::zoomX(2)) *
                                        mapW &&
                            msg.y <=
                                LGGraphics::windowData.maplocY +
                                    int(blockHeight + LGGraphics::zoomY(2)) *
                                        mapH) {
                            int lin =
                            ...
                            moveS mv{1, false, LGgame::playerFocus[1],
                                     coordS{lin, col}};
        ...
        while (kbmsg()) {
            ...
            switch (ch.key) {
                case int('w'):
                case key_up:   { /*[UP]*/
                    coordS to = coordS{LGgame::playerFocus[1].x + dx[1],
                                       LGgame::playerFocus[1].y + dy[1]};
                    moveS mv =
                        moveS{1, !shiftPressed, LGgame::playerFocus[1], to};
                    if (!LGgame::checkMove(mv)) {
                ...
                case int('a'):
                case key_left: { /*[LEFT]*/
                    coordS to = coordS{LGgame::playerFocus[1].x + dx[2],
                                       LGgame::playerFocus[1].y + dy[2]};
                    moveS mv =
                        moveS{1, !shiftPressed, LGgame::playerFocus[1], to};
                    if (!LGgame::checkMove(mv)) {
                        movement.push_back(mv);
                ...
                case int('s'):
                case key_down: { /*[DOWN]*/
                    coordS to = coordS{LGgame::playerFocus[1].x + dx[3],
                                       LGgame::playerFocus[1].y + dy[3]};
                    moveS mv =
                        moveS{1, !shiftPressed, LGgame::playerFocus[1], to};
                    if (!LGgame::checkMove(mv)) {
                        movement.push_back(mv);
                ...
                case key_right: { /*[RIGHT]*/
                    coordS to = coordS{LGgame::playerFocus[1].x + dx[4],
                                       LGgame::playerFocus[1].y + dy[4]};
                    moveS mv =
                        moveS{1, !shiftPressed, LGgame::playerFocus[1], to};
                    if (!LGgame::checkMove(mv)) {
                        movement.push_back(mv);
                ...
                case int('g'): {
                    movement.push_back(
                        {1, false, LGgame::playerFocus[1], LGgame::genCoo[1]});
                ...
                case 27: { /*[ESC]*/
                    MessageBoxW(getHWnd(),
                                wstring(L"YOU HAVE QUITTED THE GAME.").c_str(),
                                L"EXIT", MB_OK | MB_SYSTEMMODAL);
                    closegraph();
                    return 0;
                }
                case int('\b'): {
                    if (!LGgame::isAlive[1]) break;
                    std::chrono::nanoseconds bg =
                        std::chrono::steady_clock::now().time_since_epoch();
                    int confirmSur = MessageBoxW(
                        getHWnd(),
                        wstring(L"ARE YOU SURE TO SURRENDER?").c_str(),
                        L"CONFIRM SURRENDER", MB_YESNO | MB_SYSTEMMODAL);
                    std::chrono::nanoseconds ed =
                        std::chrono::steady_clock::now().time_since_epoch();
                    LGgame::beginTime += ed - bg;
        ...
        if (toNextTurn) {
            ...
            // MessageBoxA(getHWnd(), string("TESTING").c_str(), "TEST
            // MESSAGE", MB_OK);
            for (int i = 2; i <= LGgame::playerCnt; ++i) {
                if (!LGgame::isAlive[i]) continue;
                tmpcoo = LGgame::playerFocus[i];
                switch (LGgame::robotId[i]) {
                    case 0 ... 99:
                        mv = smartRandomBot::calcNextMove(
                            i, LGgame::playerFocus[i]);
                        if (!LGgame::checkMove(mv)) {
                            LGreplay::Movement mov(mv);
                            LGreplay::wreplay.newMove(mov);
                            LGgame::inlineMove.push_back(mv);
                ...
                if (LGset::gameMode == 1) {
                    for (int id = 1; id <= LGgame::playerCnt; ++id) {
                        if (LGgame::gameStats[id].back().city <= 0) {
                            ...
                            for (int i = 1; i <= mapH; ++i)
                                for (int j = 1; j <= mapW; ++j)
                                    if (gmp(i, j) == id) gmp(i, j) = 0;
                        }
                    }
                } else if (LGset::gameMode == 2) {
                    for (int id = 1; id <= LGgame::playerCnt; ++id) {
                        if (LGgame::gameStats[id].back().gtot() <= 0)
                            LGgame::isAlive[id] = 0;
                    }
                }
            }
        ...
        if (LGgame::cheatCode != 1048575) {
            ...
            if (!alldead) {
                ...
                MessageBoxW(
                    nullptr,
                    L"ALL THE PLAYERS YOU SELECTED TO BE SEEN ARE "
                    L"DEAD.\nTHE OVERALL CHEAT MODE WILL BE SWITCHED ON.",
                    L"TIP", MB_OK | MB_SYSTEMMODAL);
                std::chrono::nanoseconds ed =
                    std::chrono::steady_clock::now().time_since_epoch();
                LGgame::beginTime += ed - bg;
            }
        ...
        if (!gameEnd) {
            ...
            if (__builtin_popcount(ed) == 1) {
                std::chrono::nanoseconds bg =
                    std::chrono::steady_clock::now().time_since_epoch();
                MessageBoxW(nullptr,
                            (L"PLAYER " + playerInfo[std::__lg(ed)].name +
                             L" WON!" + L"\n" + L"THE GAME WILL CONTINUE." +
                             L"\n" + L"YOU CAN PRESS [ESC] TO EXIT.")
                                .c_str(),
                            L"GAME OVER", MB_OK | MB_SYSTEMMODAL);
                std::chrono::nanoseconds eed =
                    std::chrono::steady_clock::now().time_since_epoch();
        ...
        if (1) {
            ...
            std::chrono::nanoseconds timePassed =
                std::chrono::steady_clock::now().time_since_epoch() -
                LGgame::beginTime;
            int needFlushToTurn = ceill(timePassed.count() / 1'000'000'000.0L *
                                        LGgame::gameSpeed);
            ...
            if (lackTurn <= 0 || gamePaused) {
                ...
                setfillcolor(LGGraphics::bgColor);
                bar(screenszr - rspeedlen - 10 - fpslen - 10 - turnlen - 10, 0,
                    screenszr, 20 * LGGraphics::windowData.zoomY);
                ...
                setfillcolor(RED);
                setfillcolor(GREEN);
                bar(screenszr - rspeedlen - 10, 0, screenszr,
                    20 * LGGraphics::windowData.zoomY);
                rectangle(screenszr - rspeedlen - 10, 0, screenszr,
                          20 * LGGraphics::windowData.zoomY);
                setfillcolor(RED);
                bar(screenszr - rspeedlen - 10 - fpslen - 10, 0,
                    screenszr - rspeedlen - 10,
                    20 * LGGraphics::windowData.zoomY);
                rectangle(screenszr - rspeedlen - 10 - fpslen - 10, 0,
                          screenszr - rspeedlen - 10,
                          20 * LGGraphics::windowData.zoomY);
                setfillcolor(BLUE);
                bar(screenszr - rspeedlen - 10 - fpslen - 10 - turnlen - 10, 0,
                    screenszr - rspeedlen - 10 - fpslen - 10,
                    20 * LGGraphics::windowData.zoomY);
                rectangle(
                    screenszr - rspeedlen - 10 - fpslen - 10 - turnlen - 10, 0,
                    screenszr - rspeedlen - 10 - fpslen - 10,
                    20 * LGGraphics::windowData.zoomY);
                settextjustify(CENTER_TEXT, TOP_TEXT);
                ...
                timePassed =
                    std::chrono::steady_clock::now().time_since_epoch() -
                    LGgame::beginTime;
                needFlushToTurn = ceil(timePassed.count() / 1'000'000'000.0L *
                                       LGgame::gameSpeed);
