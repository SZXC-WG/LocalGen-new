	...
	WelcomePageStartLabel:;
		...
		for(; is_run(); delay_fps(120)) {
			...
			if(settings.status == 2) {
				settingsPage(); goto WelcomePageStartLabel;
			}
			if(repo.status == 2) {
				repo.clickEvent();;
			}
	...
	void webOptions() {
		...
		for(; is_run(); delay_fps(120)) {
			while(mousemsg()) {
				...
				if(msg.x >= 400 * windowData.zoomX && msg.x <= 600 * windowData.zoomY
				   && msg.y >= 350 * windowData.zoomY && msg.y <= 450 * windowData.zoomY) {
					serverBox.status = 1;
				...
				if(msg.x >= 800 * windowData.zoomX && msg.x <= 1000 * windowData.zoomY
				   && msg.y >= 350 * windowData.zoomY && msg.y <= 450 * windowData.zoomY) {
					clientBox.status = 1;
