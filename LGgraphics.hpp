...
void initWindowSize() {
    ...
    xyprintf(500 + getwidth(iconImg) / 2, 10 + getheight(iconImg) + 20 / 2 + 10,
             L"version %d.%d.%d (build %d)", VER_MAJOR, VER_MINOR, VER_RELEASE,
             VER_BUILD);
    delimage(iconImg);
    ...
    }
    {
        ...
        scrsz[i].addtext(L"Full Screen (" +
                         to_wstring(GetSystemMetrics(SM_CXSCREEN)) + L" × " +
                         to_wstring(GetSystemMetrics(SM_CYSCREEN)) + L")");
        scrsz[i].clickEvent = [i]() -> void { select = i; };
        scrsz[i].textalign(CENTER_TEXT, CENTER_TEXT);
        scrsz[i].display();
    }
    ...
    for (; is_run(); delay_fps(120)) {
        ...
        for (int i = 1; i <= 12; ++i) {
            ...
            for (auto x : LGgame::team[i])
                width[i] =
                    max(width[i], textwidth(LGgame::playerNames[x].c_str()));
            ...
            rowHeight[totRows] =
                max(rowHeight[totRows], int(32 + 15 * LGgame::team[i].size()));
        }
        rowHeight[0] = 10;
        ...
        for (int i = 12; i; --i) {
            ...
            setfont(25, 0, LGset::mainFontName.c_str());
            settextjustify(CENTER_TEXT, CENTER_TEXT);
            xyprintf(rowWidth[irow[i]] - (width[i] + 25) / 2 + begWid[irow[i]],
...
void localOptions() {
    ...
    for (; is_run(); delay_fps(120)) {
        ...
        while (mousemsg()) {
            int id =
                int((msg.y / windowData.zoomY - 100 - shiftval) / butH) * 5 +
            ...
            if (msg.x < 50 * windowData.zoomX ||
                ...
                continue;
            id = int((msg.y / windowData.zoomY - 100 - shiftval) / butH) * 5 +
                 int((msg.x / windowData.zoomX - 50) / 300) + 1;
        ...
        for (int i = 1; i <= mapNum; ++i) {
            mapbut[i]
                ...
                .addtext(L"Plain Count: " +
                         (~mapInfo[i].plaincnt ? to_wstring(mapInfo[i].plaincnt)
                                               : L"undetermined"))
                .addtext(L"City Count: " + (~mapInfo[i].citycnt
                ...
                .addtext(L"Swamp Count: " +
                         (~mapInfo[i].swampcnt ? to_wstring(mapInfo[i].swampcnt)
                                               : L"undetermined"))
                .addtext(L"Size: " +
...
void replayPage() {
    ...
    jumpbut.textalign(CENTER_TEXT, CENTER_TEXT)
        .bgcolor(WHITE)
        .textcolor(BLACK)
        .move(0 * windowData.zoomX, 0 * windowData.zoomY)
        .fontname(LGset::mainFontName.c_str())
        .fontsize(24 * windowData.zoomY, 0)
        .size(160 * windowData.zoomX, 30 * windowData.zoomY)
        .addtext(L"Jump to turn: ")
        .enableTextShadow = false;
    jumpbox.create(false);
    jumpbox.size(60 * windowData.zoomX, 30 * windowData.zoomY);
    jumpbox.move(160 * windowData.zoomX, 0 * windowData.zoomY);
    jumpbox.setfont(22 * windowData.zoomY, 0, LGset::mainFontName.c_str());
    jumpbox.visible(true);
    jumpsmbut.textalign(CENTER_TEXT, CENTER_TEXT)
        .bgcolor(WHITE)
        .textcolor(BLACK)
        .move(220 * windowData.zoomX, 0 * windowData.zoomY)
        .fontname(LGset::mainFontName.c_str())
        .fontsize(30 * windowData.zoomY, 0)
        .size(50 * windowData.zoomX, 30 * windowData.zoomY)
        .addtext(L"→")
        .enableTextShadow = false;
    jumpsmbut.enableButtonShadow = false;
    ...
    for (; is_run();) {
        ...
        rectangle(screenszr - fpslen - 10, 0, screenszr,
                  20 * LGGraphics::windowData.zoomY);
        setfillcolor(BLUE);
        bar(screenszr - fpslen - 10 - turnlen - 10, 0, screenszr - fpslen - 10,
            20 * LGGraphics::windowData.zoomY);
...
void createMapPage() {
    ...
    for (; is_run(); delay_fps(120)) {
                ...
                if (msg.is_down()) {
                    ...
                    if (!moved && now - prsttm < 200ms) {
                        if (msg.x < zoomX(40) &&
                            ...
                            msg.y < scrh / 2 + zoomY(200))
                            type =
                                (msg.y - (scrh / 2 - zoomY(200))) / zoomY(40);
                            ...
                            bar(scrw / 2 - zoomX(130), scrh / 2 - zoomY(80),
                                scrw / 2 + zoomX(130), scrh / 2 + zoomY(80));
                            ...
                            for (; is_run(); delay_fps(120)) {
                                ...
                                if (loadButton.status == 2) {
                                    ...
                                    string ss = s;
                                    FILE* file;
                                    file = fopen(("maps/" + ss + ".lg").c_str(),
                                                 "r");
                                    if (!file) {
                                        MessageBoxA(nullptr, "Map not found!",
                                ...
                                for (int i = 1; i <= mapH; ++i)
                                   ...
                                   msg.y <= LGGraphics::windowData.maplocY +
                                                int(blockHeight +
                                                    ...
                                                    mapH) {
                            int lin =
                                (msg.y + blockHeight + LGGraphics::zoomX(2) -
                                 1 - LGGraphics::windowData.maplocY) /
                                int(blockHeight + LGGraphics::zoomX(2));
                            int col =
                                (msg.x + blockWidth + LGGraphics::zoomY(2) - 1 -
        ...
        if (type == 0) {
            bar(scrw / 2 - zoomX(105), scrh - zoomY(110), scrw / 2 + zoomX(105),
                scrh - zoomY(80));
            xyprintf(scrw / 2, scrh - zoomY(95),
                     "Click a tile to place a mountain.");
        }
        if (type == 1) {
            bar(scrw / 2 - zoomX(105), scrh - zoomY(130), scrw / 2 + zoomX(105),
                scrh - zoomY(80));
        ...
        if (type == 5) {
            bar(scrw / 2 - zoomX(95), scrh - zoomY(110), scrw / 2 + zoomX(95),
                scrh - zoomY(80));
            xyprintf(scrw / 2, scrh - zoomY(95),
                     "Click a tile to toggle light tile.");
        }
        if (type == 6) {
            bar(scrw / 2 - zoomX(80), scrh - zoomY(110), scrw / 2 + zoomX(80),
                scrh - zoomY(80));
            xyprintf(scrw / 2, scrh - zoomY(95), "Click a tile to remove it.");
        }
        if (type == 3) {
            citynumBox.visible(true);
            bar(scrw / 2 - zoomX(85), scrh - zoomY(110), scrw / 2 + zoomX(5),
                scrh - zoomY(80));
            ...
            citynumBox.visible(false);
        if (type == 4) {
            plainnumBox.visible(true);
            bar(scrw / 2 - zoomX(115), scrh - zoomY(110), scrw / 2 + zoomX(35),
                scrh - zoomY(80));
...
void doMapSelect() {
    ...
    xyprintf(255 * windowData.zoomX, 115 * windowData.zoomY, L"Size: %d × %d",
             mapInfo[mapSelected].height, mapInfo[mapSelected].width);
    xyprintf(255 * windowData.zoomX, 145 * windowData.zoomY,
             L"General Count: %d", mapInfo[mapSelected].generalcnt);
    xyprintf(255 * windowData.zoomX, 175 * windowData.zoomY, L"Plain Count: %d",
             mapInfo[mapSelected].plaincnt);
    xyprintf(255 * windowData.zoomX, 205 * windowData.zoomY, L"City Count: %d",
             mapInfo[mapSelected].citycnt);
    xyprintf(255 * windowData.zoomX, 235 * windowData.zoomY,
             L"Mountain Count: %d", mapInfo[mapSelected].mountaincnt);
    xyprintf(255 * windowData.zoomX, 265 * windowData.zoomY, L"Swamp Count: %d",
             mapInfo[mapSelected].swampcnt);
...
void importGameSettings() {
    ...
    checkOA.size(400 * windowData.zoomX - 1 * 2, 100 * windowData.zoomY - 1 * 2)
