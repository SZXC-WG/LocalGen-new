...
bool FullScreen(HWND hwnd, int fullscreenWidth, int fullscreenHeight,
                int colourBits, int refreshRate) {
    ...
    fullscreenSettings.dmFields =
        DM_PELSWIDTH | DM_PELSHEIGHT | DM_BITSPERPEL | DM_DISPLAYFREQUENCY;

    SetWindowLongPtr(hwnd, GWL_EXSTYLE, WS_EX_APPWINDOW);
    SetWindowLongPtr(hwnd, GWL_STYLE,
                     GetWindowLongPtr(hwnd, GWL_STYLE) ^ WS_CAPTION);
    SetWindowPos(hwnd, HWND_NOTOPMOST, 0, 0, fullscreenWidth, fullscreenHeight,
                 SWP_SHOWWINDOW);
    isChangeSuccessful =
...
inline void ege_circle(int x, int y, int r) {
    ege_ellipse(x - r, y - r, r << 1, r << 1);
}
inline void ege_fillcircle(int x, int y, int r) {
    ege_fillellipse(x - r, y - r, r << 1, r << 1);
}
...
namespace LGGraphics {
    ...
    void initWindowSize() {
        ...
        xyprintf(500 + getwidth(iconImg) / 2,
                 10 + getheight(iconImg) + 20 / 2 + 10,
                 L"version %d.%d.%d (build %d)", VER_MAJOR, VER_MINOR,
                 VER_RELEASE, VER_BUILD);
        delimage(iconImg);
        ...
        for (int i = 0; i < wwlen; ++i) {
            ...
            scrsz[i].addtext(to_wstring(window_width[i]) + L" × " +
                             to_wstring(window_width[i] * 9 / 16));
        ...
        }
        {
            ...
            scrsz[i].addtext(
                L"Full Screen (" + to_wstring(GetSystemMetrics(SM_CXSCREEN)) +
                L" × " + to_wstring(GetSystemMetrics(SM_CYSCREEN)) + L")");
            scrsz[i].clickEvent = [i]() -> void { select = i; };
            scrsz[i].textalign(CENTER_TEXT, CENTER_TEXT);
            scrsz[i].display();
        }
        for (; is_run(); delay_fps(60)) {
            for (int i = 0; i <= wwlen; ++i) {
                ...
                scrsz[i].display();
                if (scrsz[i].status == 2)
                    scrsz[i].clickEvent(), changeMade = false;
            }
            if (!changeMade) break;
        }
    finishSelect:
        if (select == 12) {
            ...
            windowData.zoomX = windowData.zoomY =
                (double)window_width[select] / 1600.0;
            int nScreenWidth, nScreenHeight;
            nScreenWidth  = GetSystemMetrics(SM_CXSCREEN);
            nScreenHeight = GetSystemMetrics(SM_CYSCREEN);
            initgraph(1600 * windowData.zoomX, 900 * windowData.zoomY,
                      RENDER_AUTO);
        }
        ...
        for (; is_run(); delay_fps(120)) {
            ...
            if (submitButton.status == 2) {
                ...
                if (checkValidUsername(username)) return;
                    ...
                    bar(scrw / 2 - 110, scrh / 2 + 5, scrw / 2 + 110,
                        scrh / 2 + 25);
                    setcolor(BLACK);
                    setfont(15, 0, LGset::mainFontName.c_str());
                    xyprintf(scrw / 2, scrh / 2 + 15,
                             "Username must be between 3 and 16 letters.");
                }
            }
        }
    }
    ...
    WelcomePageStartLabel:;
        ...
        setfont(150 * windowData.zoomY, 0, LGset::mainFontName.c_str(), 0, 0,
                FW_BOLD, 0, 0, 0);
        setcolor(WHITE);
        ...
        xyprintf(330 * windowData.zoomX, 750 * windowData.zoomY,
                 "version %d.%d.%d (build %d)", VER_MAJOR, VER_MINOR,
                 VER_RELEASE, VER_BUILD);
        ...
        local.size(600 * windowData.zoomX, 100 * windowData.zoomY)
            ...
            .display();
        web.size(600 * windowData.zoomX, 100 * windowData.zoomY)
            ...
            .frame(10 * windowData.zoomY)
            .display();
        replay.size(600 * windowData.zoomX, 100 * windowData.zoomY)
            ...
            .frame(10 * windowData.zoomY)
            .display();
        createmap.size(600 * windowData.zoomX, 100 * windowData.zoomY)
            ...
            .frame(10 * windowData.zoomY)
            .display();
        donate.size(400 * windowData.zoomX, 50 * windowData.zoomY)
            ...
            .event([]() {
                ...
                images::zoomImage(donate_ap, 600 * windowData.zoomX,
                                  800 * windowData.zoomY);
                putimage(100 * windowData.zoomX, 10 * windowData.zoomY,
                         donate_wc);
                putimage((100 + 800) * windowData.zoomX, 10 * windowData.zoomY,
                         donate_ap);
                xyprintf(800 * windowData.zoomX, 830 * windowData.zoomY,
            ...
            .display();
        settings.radius(50 * min(windowData.zoomX, windowData.zoomY))
        ...
        settings.display();
        repo.radius(50 * min(windowData.zoomX, windowData.zoomY))
            ...
            .frame(10 * windowData.zoomY)
            .event([]() -> void {
                system("start http://github.com/SZXC-WG/LocalGen-new");
            })
            .enableButtonShadow = false;
        repo.display();
    ...
    void testPage() {
        ...
        for (; is_run(); delay_fps(120)) {
            ...
            int width[13], irow[13], rowWidth[13], rowHeight[13], totRows = 0,
                                                                  begWid[13];
            rowWidth[0] = 3000;
            for (int i = 1; i <= 12; ++i) {
                width[i] = 0;
                ...
                setfont(15, 0, LGset::mainFontName.c_str());
                for (auto x : LGgame::team[i])
                    width[i] = max(width[i],
                                   textwidth(LGgame::playerNames[x].c_str()));
                ...
                rowHeight[totRows] = max(rowHeight[totRows],
                                         int(32 + 15 * LGgame::team[i].size()));
            }
            rowHeight[0] = 10;
            ...
            for (int i = 12; i; --i) {
                ...
                bar(rowWidth[irow[i]] - width[i] - 25 + begWid[irow[i]],
                    rowHeight[irow[i] - 1] + 110,
                    rowWidth[irow[i]] + begWid[irow[i]],
                    rowHeight[irow[i] - 1] + 15 * LGgame::team[i].size() + 32 +
                        110);
                setcolor(WHITE);
                setfont(25, 0, LGset::mainFontName.c_str());
                settextjustify(CENTER_TEXT, CENTER_TEXT);
                xyprintf(
                    rowWidth[irow[i]] - (width[i] + 25) / 2 + begWid[irow[i]],
                    rowHeight[irow[i] - 1] + 12 + 110,
                    ("Team " + to_string(i)).c_str());
                setfont(15, 0, LGset::mainFontName.c_str());
                settextjustify(LEFT_TEXT, TOP_TEXT);
                for (int j = 0; j < LGgame::team[i].size(); ++j) {
                    setfillcolor(playerInfo[LGgame::team[i][j]].color);
                    bar(rowWidth[irow[i]] - width[i] - 25 + 5 + 2 +
                            begWid[irow[i]],
                        rowHeight[irow[i] - 1] + 5 + 20 + j * 15 + 2 + 110,
                        rowWidth[irow[i]] - width[i] - 25 + 5 + 13 +
                            begWid[irow[i]],
                        rowHeight[irow[i] - 1] + 5 + 20 + j * 15 + 13 + 110);
                    xyprintf(rowWidth[irow[i]] - width[i] - 25 + 20 +
                                 begWid[irow[i]],
                             rowHeight[irow[i] - 1] + 5 + 20 + j * 15 + 110,
                             LGgame::playerNames[LGgame::team[i][j]].c_str());
                }
    ...
    void localOptions() {
        ...
        for (int i = 1; i <= mapNum; ++i) {
            mapbut[i]
                ...
                .move(((i - 1) % 5 * 300 + 50) * windowData.zoomX,
                      ((i - 1) / 5 * butH + 100 + shiftval) * windowData.zoomY)
                .bgcolor(bgColor)
                .textcolor(WHITE)
                .textalign(CENTER_TEXT, CENTER_TEXT)
                .fontname(LGset::mainFontName.c_str())
                ...
                // .addtext("Size: " + to_string(maps[i].hei) + " × " +
                // to_string(maps[i].wid))
                .display();
        }
        settextjustify(CENTER_TEXT, TOP_TEXT);
        setcolor(WHITE);
        ...
        for (; is_run(); delay_fps(120)) {
            ...
            while (mousemsg()) {
                int id =
                    int((msg.y / windowData.zoomY - 100 - shiftval) / butH) *
                        5 +
                    int((msg.x / windowData.zoomX - 50) / 300) + 1;
                mapbut[id].status = 0;
                msg               = getmouse();
                shiftval += msg.wheel;
                if (shiftval > 0) shiftval = 0;
                if (shiftval < -((mapNum - 1) / 5 * butH))
                    shiftval = -((mapNum - 1) / 5 * butH);
                if (msg.x < 50 * windowData.zoomX ||
                    msg.x > 1550 * windowData.zoomX ||
                    msg.y < 100 * windowData.zoomY ||
                    msg.y > 900 * windowData.zoomY)
                    continue;
                id = int((msg.y / windowData.zoomY - 100 - shiftval) / butH) *
                         5 +
                     int((msg.x / windowData.zoomX - 50) / 300) + 1;
            ...
            for (int i = 1; i <= mapNum; ++i) {
                mapbut[i]
                    ...
                    .addtext(L"General Count: " +
                             (~mapInfo[i].generalcnt
                                  ? to_wstring(mapInfo[i].generalcnt)
                                  : L"2~12"))
                    .addtext(L"Plain Count: " +
                             (~mapInfo[i].plaincnt
                                  ? to_wstring(mapInfo[i].plaincnt)
                                  : L"undetermined"))
                    .addtext(L"City Count: " +
                             (~mapInfo[i].citycnt
                                  ? to_wstring(mapInfo[i].citycnt)
                                  : L"undetermined"))
                    .addtext(L"Mountain Count: " +
                             (~mapInfo[i].mountaincnt
                                  ? to_wstring(mapInfo[i].mountaincnt)
                                  : L"undetermined"))
                    .addtext(L"Swamp Count: " +
                             (~mapInfo[i].swampcnt
                                  ? to_wstring(mapInfo[i].swampcnt)
                                  : L"undetermined"))
                    .addtext(L"Size: " +
                             (~mapInfo[i].height ? to_wstring(mapInfo[i].height)
                                                 : L"undetermined") +
                             L" × " +
                             (~mapInfo[i].width ? to_wstring(mapInfo[i].width)
                                                : L"undetermined"))
                    .move(((i - 1) % 5 * 300 + 50) * windowData.zoomX,
                          ((i - 1) / 5 * butH + 100 + shiftval) *
                              windowData.zoomY)
                    .display();
            ...
            line(0, 100 * windowData.zoomY, 1600 * windowData.zoomX,
                 100 * windowData.zoomY);
    ...
    void webOptions() {
        ...
        serverBox.size(200 * windowData.zoomX, 100 * windowData.zoomY)
        ...
        clientBox.size(200 * windowData.zoomX, 100 * windowData.zoomY)
        ...
        for (; is_run(); delay_fps(120)) {
            while (mousemsg()) {
                ...
                if (msg.x >= 400 * windowData.zoomX &&
                    msg.x <= 600 * windowData.zoomY &&
                    msg.y >= 350 * windowData.zoomY &&
                    msg.y <= 450 * windowData.zoomY) {
                    serverBox.status = 1;
                ...
                if (msg.x >= 800 * windowData.zoomX &&
                    msg.x <= 1000 * windowData.zoomY &&
                    msg.y >= 350 * windowData.zoomY &&
                    msg.y <= 450 * windowData.zoomY) {
                    clientBox.status = 1;
    ...
    serverOptions:;
        ...
        for (int i = 1; i <= mapNum; ++i) {
            mapbut[i]
                ...
                .move(((i - 1) % 4 * 300) * windowData.zoomX,
                      ((i - 1) / 4 * 200 + shiftval) * windowData.zoomY)
                .bgcolor(bgColor)
                .textcolor(WHITE)
                .textalign(CENTER_TEXT, CENTER_TEXT)
                .fontname(LGset::mainFontName.c_str())
        ...
        impfin.textalign(CENTER_TEXT, CENTER_TEXT)
        ...
        setfont(50 * windowData.zoomY, 0, LGset::mainFontName.c_str());
        delay_ms(50);
        xyprintf(1400 * windowData.zoomX, 50 * windowData.zoomY,
                 "or import a map...");
        delay_ms(0);
        delay_ms(50);
        flushmouse();
        ...
        for (; is_run(); delay_fps(120)) {
            while (mousemsg()) {
                int id = int((msg.y - shiftval * windowData.zoomX) /
                             (200 * windowData.zoomY)) *
                             4 +
                         int(msg.x / (300 * windowData.zoomX)) + 1;
                mapbut[id].status = 0;
                impfin.status     = 0;
                msg               = getmouse();
                shiftval += msg.wheel;
                if (shiftval > 0) shiftval = 0;
                if (shiftval < -(mapNum - 1) / 4 * 200)
                    shiftval = -(mapNum - 1) / 4 * 200;
                if (msg.x >= 1250 * windowData.zoomX &&
                    msg.x <= 1550 * windowData.zoomY &&
                    msg.y >= 825 * windowData.zoomY &&
                    msg.y <= 875 * windowData.zoomY) {
                    impfin.status = 1;
                    if (msg.is_left()) impfin.status = 2;
                    continue;
                }
                if (msg.x < 0 || msg.x > 1200 * windowData.zoomX || msg.y < 0 ||
                    msg.y > 900 * windowData.zoomY)
                    continue;
                id = int((msg.y - shiftval * windowData.zoomX) /
                         (200 * windowData.zoomY)) *
                         4 +
                     int(msg.x / (300 * windowData.zoomX)) + 1;
    ...
    void replayPage() {
        ...
        for (; is_run();) {
            ...
            while (kbmsg()) {
                ...
                switch (ch.key) {
                    case key_esc: { /*[ESC]*/ closegraph(); return;
                    }
            ...
            bar(screenszr - 10 - fpslen - 10 - turnlen - 10, 0, screenszr,
                20 * LGGraphics::windowData.zoomY);
            setfont(20 * LGGraphics::windowData.zoomY, 0,
                    LGset::mainFontName.c_str());
            fpslen  = textwidth(("FPS: " + to_string(getfps())).c_str());
            turnlen = textwidth(
                ("Turn " + to_string(LGreplay::rreplay.curTurn) + ".").c_str());
            setfillcolor(RED);
            bar(screenszr - fpslen - 10, 0, screenszr,
                20 * LGGraphics::windowData.zoomY);
            rectangle(screenszr - fpslen - 10, 0, screenszr,
                      20 * LGGraphics::windowData.zoomY);
            setfillcolor(BLUE);
            bar(screenszr - fpslen - 10 - turnlen - 10, 0,
                screenszr - fpslen - 10, 20 * LGGraphics::windowData.zoomY);
            rectangle(screenszr - fpslen - 10 - turnlen - 10, 0,
                      screenszr - fpslen - 10,
                      20 * LGGraphics::windowData.zoomY);
            settextjustify(CENTER_TEXT, TOP_TEXT);
            xyprintf(screenszr - fpslen / 2 - 5, 0, "FPS: %f", getfps());
            xyprintf(screenszr - fpslen - 10 - turnlen / 2 - 5, 0, "Turn %d.",
                     LGreplay::rreplay.curTurn);
            LGgame::statistics();
    ...
    void createMapPage() {
        ...
        saveButton.size(zoomX(90), zoomY(40))
            ...
            .textalign(CENTER_TEXT, CENTER_TEXT)
            .addtext(L"Save");
        cancelButton.size(zoomX(90), zoomY(40))
            ...
            .textalign(CENTER_TEXT, CENTER_TEXT)
            .addtext(L"Cancel");
        loadButton.size(zoomX(90), zoomY(40))
            .move(scrw / 2 - zoomX(100), scrh / 2 + zoomY(30))
            .fontname(LGset::mainFontName.c_str())
            .fontsize(zoomY(40), 0)
            .bgcolor(mainColor)
        ...
        int smsx = 0, smsy = 0, midact = 0, type = 0, citynum = 40,
            plainnum                              = 40;
        ...
        for (; is_run(); delay_fps(120)) {
                ...
                if (msg.is_move()) {
                    ...
                    if (msg.is_down()) {
                        ...
                        if (!moved && now - prsttm < 200ms) {
                            if (msg.x < zoomX(40) &&
                                msg.y >= scrh / 2 - zoomY(200) &&
                                msg.y < scrh / 2 + zoomY(200))
                                type = (msg.y - (scrh / 2 - zoomY(200))) /
                                       zoomY(40);
                            else if (msg.x >= scrw / 2 - zoomX(145) &&
                                     msg.x < scrw / 2 - zoomX(5) &&
                                     msg.y >= scrh - zoomY(60) &&
                                     msg.y < scrh - zoomY(20)) {
                                settextjustify(CENTER_TEXT, CENTER_TEXT);
                                setfillcolor(WHITE);
                                setcolor(BLACK);
                                bar(scrw / 2 - zoomX(130), scrh / 2 - zoomY(80),
                                    scrw / 2 + zoomX(130),
                                    scrh / 2 + zoomY(80));
                                setfont(zoomY(40), 0,
                                        LGset::mainFontName.c_str());
                                xyprintf(scrw / 2, scrh / 2 - zoomY(50),
                                         "Save map");
                                xyprintf(scrw / 2 - zoomX(55), scrh / 2,
                                         "Map name:");
                            ...
                            } else if (msg.x >= scrw / 2 + zoomX(5) &&
                                       msg.x < scrw / 2 + zoomX(145) &&
                                       msg.y >= scrh - zoomY(60) &&
                                       msg.y < scrh - zoomY(20)) {
                                settextjustify(CENTER_TEXT, CENTER_TEXT);
                                setfillcolor(WHITE);
                                ...
                                for (; is_run(); delay_fps(120)) {
                                    ...
                                    if (loadButton.status == 2) {
                                        ...
                                        if (!file) {
                                            MessageBoxA(
                                                nullptr, "Map not found!",
                                                "Local Generals", MB_OK);
                                            continue;
                                        }
                                ...
                                savenameBox.visible(false);
                            } else if (msg.x >= scrw / 2 - zoomX(200) &&
                                       msg.x < scrw / 2 - zoomX(170) &&
                                       msg.y >= zoomY(10) &&
                                       msg.y < zoomY(40)) {
                                if (mapH > 1) --mapH;
                            } else if (msg.x >= scrw / 2 - zoomX(50) &&
                                       msg.x < scrw / 2 - zoomY(20) &&
                                       msg.y >= zoomY(10) &&
                                       msg.y < zoomY(40)) {
                                if (mapH < 100) {
                                    ++mapH;
                                    for (int i = 1; i <= mapW; ++i)
                                        gameMap[mapH][i] = { 0, 0, 0, 0 };
                                }
                            } else if (msg.x >= scrw / 2 + zoomX(20) &&
                                       msg.x < scrw / 2 + zoomX(50) &&
                                       msg.y >= zoomY(10) &&
                                       msg.y < zoomY(40)) {
                                if (mapW > 1) --mapW;
                            } else if (msg.x >= scrw / 2 + zoomX(170) &&
                                       msg.x < scrw / 2 + zoomX(200) &&
                                       msg.y >= zoomY(10) &&
                                       msg.y < zoomY(40)) {
                                if (mapW < 100) {
                                    ++mapW;
                                    for (int i = 1; i <= mapH; ++i)
                                        gameMap[i][mapW] = { 0, 0, 0, 0 };
                                }
                            } else if (msg.x >=
                                           LGGraphics::windowData.maplocX &&
                                       msg.y >=
                                           LGGraphics::windowData.maplocY &&
                                       msg.x <= LGGraphics::windowData.maplocX +
                                                    int(blockWidth +
                                                        LGGraphics::zoomX(2)) *
                                                        mapW &&
                                ...
                                int lin =
                                    (msg.y + blockHeight +
                                     LGGraphics::zoomX(2) - 1 -
                                     LGGraphics::windowData.maplocY) /
                                    int(blockHeight + LGGraphics::zoomX(2));
                                int col =
                                    (msg.x + blockWidth + LGGraphics::zoomY(2) -
                                     1 - LGGraphics::windowData.maplocX) /
                                    int(blockWidth + LGGraphics::zoomY(2));
                                switch (type) {
                                    case 0:
            ...
            if (type == 0) {
                bar(scrw / 2 - zoomX(105), scrh - zoomY(110),
                    scrw / 2 + zoomX(105), scrh - zoomY(80));
                xyprintf(scrw / 2, scrh - zoomY(95),
                         "Click a tile to place a mountain.");
            }
            if (type == 1) {
                bar(scrw / 2 - zoomX(105), scrh - zoomY(130),
                    scrw / 2 + zoomX(105), scrh - zoomY(80));
                xyprintf(scrw / 2, scrh - zoomY(115),
                         "Click a tile to place a swamp.");
                xyprintf(scrw / 2, scrh - zoomY(95),
                         "Swamps drain 1 army per turn.");
            }
            if (type == 5) {
                bar(scrw / 2 - zoomX(95), scrh - zoomY(110),
                    scrw / 2 + zoomX(95), scrh - zoomY(80));
                xyprintf(scrw / 2, scrh - zoomY(95),
                         "Click a tile to toggle light tile.");
            }
            if (type == 6) {
                bar(scrw / 2 - zoomX(80), scrh - zoomY(110),
                    scrw / 2 + zoomX(80), scrh - zoomY(80));
                xyprintf(scrw / 2, scrh - zoomY(95),
                         "Click a tile to remove it.");
            }
            if (type == 3) {
                citynumBox.visible(true);
                bar(scrw / 2 - zoomX(85), scrh - zoomY(110),
                    scrw / 2 + zoomX(5), scrh - zoomY(80));
                xyprintf(scrw / 2 - zoomX(40), scrh - zoomY(95),
                         "City Strength:");
                char s[10];
            ...
            if (type == 4) {
                ...
                bar(scrw / 2 - zoomX(115), scrh - zoomY(110),
                    scrw / 2 + zoomX(35), scrh - zoomY(80));
                xyprintf(scrw / 2 - zoomX(40), scrh - zoomY(95),
    ...
    void doMapSelect() {
        ...
        setlinewidth(1);
        setcolor(mainColor);
        setfont(40 * windowData.zoomY, 0, LGset::mainFontName.c_str(), 0, 0,
                FW_BOLD, false, false, false);
        settextjustify(CENTER_TEXT, TOP_TEXT);
        xyprintf(255 * windowData.zoomX, 5 * windowData.zoomY, L"%ls",
                 mapInfo[mapSelected].chiname.c_str());
        xyprintf(255 * windowData.zoomX, 45 * windowData.zoomY, L"%ls",
                 mapInfo[mapSelected].engname.c_str());
        setcolor(WHITE);
        setfont(30 * windowData.zoomY, 0, LGset::mainFontName.c_str());
        xyprintf(255 * windowData.zoomX, 85 * windowData.zoomY, L"Author: %ls",
                 mapInfo[mapSelected].auth.c_str());
        xyprintf(255 * windowData.zoomX, 115 * windowData.zoomY,
                 L"Size: %d × %d", mapInfo[mapSelected].height,
                 mapInfo[mapSelected].width);
        ...
        if (mapSelected < 9) {
            ...
            xyprintf(800 * windowData.zoomX, 5 * windowData.zoomY,
                     L"Input Height (<=100):");
            xyprintf(800 * windowData.zoomX, 45 * windowData.zoomY,
                     L"Input Width (<=100):");
            if (mapSelected == MAP_CITY_ID) {
                xyprintf(800 * windowData.zoomX, 85 * windowData.zoomY,
                         L"Input MINIMUM Army:");
                xyprintf(800 * windowData.zoomX, 125 * windowData.zoomY,
                         L"Input MAXIMUM Army:");
            }
            ...
            heiinput.setfont(35 * windowData.zoomY, 0,
                             LGset::mainFontName.c_str());
            ...
            widinput.setfont(35 * windowData.zoomY, 0,
                             LGset::mainFontName.c_str());
            if (mapSelected == MAP_CITY_ID) {
                ...
                amninput.setfont(35 * windowData.zoomY, 0,
                                 LGset::mainFontName.c_str());
                ...
                amxinput.setfont(35 * windowData.zoomY, 0,
                                 LGset::mainFontName.c_str());
            }
            ...
            heib.move(1020 * windowData.zoomX, 6 * windowData.zoomY)
                ...
                .fontname(LGset::mainFontName.c_str())
                .addtext(L"confirm");
            widb.move(1020 * windowData.zoomX, 46 * windowData.zoomY)
                .size(100 * windowData.zoomX, 38 * windowData.zoomY)
                .bgcolor(WHITE)
                .textcolor(mainColor)
                .textalign(CENTER_TEXT, CENTER_TEXT)
                ...
                .addtext(L"confirm");
            if (mapSelected == MAP_CITY_ID) {
                amnb.move(1020 * windowData.zoomX, 86 * windowData.zoomY)
                    .size(100 * windowData.zoomX, 38 * windowData.zoomY)
                    .bgcolor(WHITE)
                    .textcolor(mainColor)
                    .textalign(CENTER_TEXT, CENTER_TEXT)
                    ...
                    .fontname(LGset::mainFontName.c_str())
                    .addtext(L"confirm");
                amxb.move(1020 * windowData.zoomX, 126 * windowData.zoomY)
                    .size(100 * windowData.zoomX, 38 * windowData.zoomY)
                    .bgcolor(WHITE)
                    .textcolor(mainColor)
                    .textalign(CENTER_TEXT, CENTER_TEXT)
            ...
            endb.move(810 * windowData.zoomX, 166 * windowData.zoomY)
            ...
            for (; is_run(); delay_fps(60)) {
                ...
                if (endb.status == 2) {
                    if (hb && wb &&
                        (mapSelected == MAP_CITY_ID ? (nb && xb) : 1)) {
                        endb.status = 0;
                        endb.display();
                        break;
                    } else {
    ...
    void importGameSettings() {
        ...
        for (int i = 2; i <= 16; ++i) {
            ...
            plCntBox[i]
                .size(100 * windowData.zoomX - 1 * 2,
                      100 * windowData.zoomY - 1 * 2)
                .move(
                    55 * windowData.zoomX + 100 * windowData.zoomX * colNum + 1,
                    400 * windowData.zoomY + 100 * windowData.zoomY * rowNum +
                        1)
                .textalign(CENTER_TEXT, CENTER_TEXT)
                .fontname(LGset::mainFontName.c_str())
        ...
        speedSubmit.size(150 * windowData.zoomX, 50 * windowData.zoomY)
        ...
        for (int i = 1; i <= 16; ++i) {
            ...
            checkBox[i]
                ...
                .size(100 * windowData.zoomX - 1 * 2,
                      100 * windowData.zoomY - 1 * 2)
                .move(1150 * windowData.zoomX +
                          100 * windowData.zoomX * colNum + 1,
                      400 * windowData.zoomY + 100 * windowData.zoomY * rowNum +
                          1);
        ...
        checkOA.display();
        gameBox.size(200 * windowData.zoomX, 100 * windowData.zoomY)
        ...
        for (; is_run(); delay_fps(120)) {
            ...
            plCnt = min(plCnt, mapInfo[mapSelected].generalcnt +
                                   mapInfo[mapSelected].plaincnt);
            setfillcolor(bgColor);
            ...
            for (int i = 1; i <= plCnt; ++i) {
                if (cheatCode >> i & 1) {
                    checkBox[i].bgcolor(playerInfo[i].color).textcolor(WHITE);
                    ;
                } else {
                    checkBox[i].bgcolor(bgColor).textcolor(playerInfo[i].color);
                    ;
                }
                int rowNum = (i - 1) / 4;
                int colNum = (i - 1) % 4;
                rectangle(
                    1150 * windowData.zoomX + 100 * windowData.zoomX * colNum,
                    400 * windowData.zoomY + 100 * windowData.zoomY * rowNum,
            ...
            if (cheatCode == 1048575) {
                checkOA.bgcolor(mainColor).textcolor(WHITE);
            } else {
                checkOA.bgcolor(bgColor).textcolor(mainColor);
            }
    ...
    void initPages() {
        ...
        tmp.locY       = zoomY(20);
        tmp.downLoc();
        tmp.info.lText->push_back(singleTextS(
            WHITE, L"Settings", LGset::mainFontName, -zoomY(50), 0));
        ...
        tmp.info.cBText->bgColor = LGGraphics::bgColor;
        tmp.info.cBText->boxText.push_back(
            singleTextS(WHITE, L"Enable analysis in game", LGset::mainFontName,
                        -zoomY(20), 0));
        tmp.info.cBText->checkBox.size(zoomX(20), zoomY(20))
            .frame(2)
            .variable(&LGset::enableAnalysisInGame)
            .bgcolor(bgColor)
            .fillcolor(WHITE)
            .framecolor(WHITE);
        tmp.info.cBText->blankWidth = zoomX(10);
        ...
        tmp.info.cButton->addtext(L"0")
            .bgcolor(LGGraphics::mainColor)
            .textcolor(WHITE)
            .fontsize(zoomY(20), 0)
            .textalign(CENTER_TEXT, CENTER_TEXT)
        ...
        tmp.downLoc();
        tmp.info.lText->push_back(singleTextS(
            WHITE, L"Modifiers", LGset::mainFontName, -zoomY(35), 0));
        p_settings.addItem(tmp);
        tmp.iType       = ITEM_RECTCHKBOX_WITH_TEXT;
        tmp.info.cBText = new rCBOXtextS;
        tmp.locX        = zoomX(800);
        ...
        tmp.info.cBText->boxText.push_back(singleTextS(
            WHITE, L"Leapfrog", LGset::mainFontName, -zoomY(20), 0));
        tmp.info.cBText->checkBox.size(zoomX(20), zoomY(20))
            .frame(2)
        ...
        tmp.info.cBText->bgColor = LGGraphics::bgColor;
        tmp.info.cBText->boxText.push_back(singleTextS(
            WHITE, L"Misty Veil", LGset::mainFontName, -zoomY(20), 0));
        tmp.info.cBText->checkBox.size(zoomX(20), zoomY(20))
            .frame(2)
        ...
        tmp.info.cBText->bgColor = LGGraphics::bgColor;
        tmp.info.cBText->boxText.push_back(singleTextS(
            WHITE, L"Silent War", LGset::mainFontName, -zoomY(20), 0));
        tmp.info.cBText->checkBox.size(zoomX(20), zoomY(20))
            .frame(2)
        ...
        tmp.info.cBText->bgColor = LGGraphics::bgColor;
        tmp.info.cBText->boxText.push_back(singleTextS(
            WHITE, L"Watchtower", LGset::mainFontName, -zoomY(20), 0));
        tmp.info.cBText->checkBox.size(zoomX(20), zoomY(20))
            .frame(2)
        ...
        tmp.info.cBText->checkBox.size(zoomX(20), zoomY(20))
            .frame(2)
            .variable(&LGset::modifier::NeutralResist)
            .bgcolor(bgColor)
            .fillcolor(WHITE)
            .framecolor(WHITE);
        tmp.info.cBText->blankWidth = zoomX(10);
