...
inline BlockView view(int Code, int x, int y) {
    if (isVisible(x, y, Code)) {
        return BlockView{true, gameMap[x][y].player, gameMap[x][y].type,
                         gameMap[x][y].army};
    } else {
        BlockView ret;
        ret.visible = false;
        ret.player = -1;
        if (unpassable(gameMap[x][y].type) ||
            gameMap[x][y].type == BLOCK_CITY) {
...
inline BlockView view(int Code, coordS position) {
    return view(Code, position.x, position.y);
}
...
bool isVisible(int x, int y, int Code) {
    ...
    if (!LGset::modifier::MistyVeil ||
        (LGset::modifier::Watchtower && gameMap[x][y].type == BLOCK_CITY)) {
        for (int i = -1; i <= 1; ++i)
            for (int j = -1; j <= 1; ++j)
    ...
    if (LGset::modifier::Watchtower) {
        for (int i = -4; i <= 4; ++i) {
            for (int j = -4 + abs(i); j <= 4 - abs(i); ++j) {
                if ((gameMap[x + i][y + j].type == 3 ||
                     gameMap[x + i][y + j].type == 4)) {
                    if (Code & (1 << gameMap[x + i][y + j].player)) return true;
                }
            }
        }
...
void printBlockNum(bool visible, long long army, int player, int curx,
                   int cury) {
    int luX = LGGraphics::windowData.maplocX +
              int(blockWidth + LGGraphics::zoomX(2)) * (cury - 1);
    int luY = LGGraphics::windowData.maplocY +
              int(blockHeight + LGGraphics::zoomY(2)) * (curx - 1);
    ...
    if (textwidth(out.c_str()) <= blockWidth - 2)
        ...
        while (out.size() > 1ull &&
               textwidth((out + ".."s).c_str()) > blockWidth - 2)
            out.pop_back();
        outtextxy(luX + blockWidth / 2, luY + blockHeight / 2,
                  (out + ".."s).c_str());
    }
}

void printMap(int Code, coordS coo) {
    static const color_t cscol = 0xff808080, plcol = 0xffdcdcdc,
                         mtcol = 0xffbbbbbb, unseen = 0xff3c3c3c;
    setcolor(WHITE);
    int blockFontSize =
        std::min(std::max(int(0.375 * blockHeight /* from the site */),
                          (int)(LGset::blockMinFontSize)),
                 (int)(LGset::blockMaxFontSize));
    // setfont(20, 0, LGset::mainFontName.c_str());
    // xyprintf(5 * LGGraphics::windowData.zoomX, 800 *
    // LGGraphics::windowData.zoomY,
    ...
    for (int curx = 1; curx <= mapH; curx++) {
        for (int cury = 1; cury <= mapW; cury++) {
            if (isVisible(curx, cury, Code)) {
                ...
                setfillcolor(unseen);
            bar(LGGraphics::windowData.maplocX +
                    int(blockWidth + LGGraphics::zoomX(2)) * (cury - 1),
                LGGraphics::windowData.maplocY +
                    int(blockHeight + LGGraphics::zoomY(2)) * (curx - 1),
                LGGraphics::windowData.maplocX +
                    int(blockWidth + LGGraphics::zoomX(2)) * cury -
                    LGGraphics::zoomX(2),
                LGGraphics::windowData.maplocY +
                    int(blockHeight + LGGraphics::zoomY(2)) * curx -
                    LGGraphics::zoomX(2));
            // ege_fillrect(widthPerBlock * (cury - 1), heightPerBlock * (curx -
            // 1), widthPerBlock, heightPerBlock);
                ...
                case 0: {
                    ...
                    if (gameMap[curx][cury].army != 0)
                        printBlockNum(isVisible(curx, cury, Code),
                                      ...
                                      gameMap[curx][cury].player, curx, cury);
                    break;
                }
                case 1: {
                    /* swamp */
...
void createOptions(int type, int h) {
    static const color_t col = 0xffdcdcdc, plcol = 0xff3c3c3c,
                         selcol = 0xff008080;
    PIMAGE npimg[15];
    for (int i = 1; i <= 13; ++i) {
        npimg[i] = newimage();
    ...
    setfillcolor(selcol);
    bar(0, h + type * LGGraphics::zoomY(40), LGGraphics::zoomX(40),
        h + LGGraphics::zoomY(40) + type * LGGraphics::zoomY(40));
    setfillcolor(plcol);
    bar(LGGraphics::zoomX(5), h + LGGraphics::zoomY(165), LGGraphics::zoomX(35),
        h + LGGraphics::zoomY(195));
    ...
    for (i = 1; i <= mapH; i++)
        for (j = 1; j <= mapW; j++) {
            ...
            if (x) {
                ...
                if (gameMap[i][j].army) gameMap[i][j].army = mtrd() % 100 + 1;
            }
        }
}
void createStandardMap(int crtH = -1, int crtW = -1) {
    ...
    for (i = 1; i <= mapH; i++)
        for (j = 1; j <= mapW; j++) {
            int x1 = 0, x2 = 0, f;
            if (i - 2 > 0 && gameMap[i - 2][j].type == 2) x1 = 1;
            if (j - 2 > 0 && gameMap[i][j - 2].type == 2) x1 = 1;
            if (i - 2 > 0 && j + 2 <= mapW && gameMap[i - 2][j + 2].type == 2)
                x1 = 1;
            if (i - 2 > 0 && j - 2 > 0 && gameMap[i - 2][j - 2].type == 2)
                x1 = 1;
...
void createMazeMap(int crtH, int crtW, int ringTYPE, int routeTYPE = 0) {
    ...
    static std::function<bool(int, int)> adjacent = [](int i, int j) -> bool {
        return (abs(p[i].x - p[j].x) + abs(p[i].y - p[j].y) == 2);
    };
    static int c, m, f[10005];
    static std::function<int(int)> getf = [](int u) -> int {
        return f[u] = (f[u] == u ? u : getf(f[u]));
    };
    static std::function<void(int, int)> unite = [](int u, int v) -> void {
...
void createFullCityMap(int crtH, int crtW, long long armyMN, long long armyMX,
                       int plCnt) {
    std::mt19937 mtrd(
        std::chrono::system_clock::now().time_since_epoch().count());
...
void getAllFiles(string path, std::vector<string>& files, string fileExt) {
    ...
    string p;
    if ((hFile = _findfirst(p.assign(path).append("\\*" + fileExt).c_str(),
                            &fileinfo)) != -1) {
...
extern int mapNum;
/*
struct MapInfoS { int id; string chiname; string engname; string auth; int hei;
int wid; int generalcnt; int swampcnt; int citycnt; int mountaincnt; int
plaincnt; MapInfoS() = default; ~MapInfoS() = default; };
*/
void initMaps() {
    mapNum = 8;
    mapInfo[1] =
        MapInfoS{1,    L"随机", L"Random", L"LocalGen", 50,   50,
                 2500, 2500,    2500,      2500,        2500, string()};
    mapInfo[2] =
        MapInfoS{2,    L"标准", L"Standard", L"LocalGen", 50,   50,
                 2500, 2500,    2500,        2500,        2500, string()};
    mapInfo[3] = MapInfoS{3,
                          L"单路（无环）迷宫",
                          L"Single-Path Maze",
                          L"LocalGen",
                          50,
                          ...
                          2500,
                          string()};
    mapInfo[4] = MapInfoS{4,           L"多环迷宫", L"Multi-Ring Maze",
                          L"LocalGen", 50,          50,
                          2500,        2500,        2500,
                          2500,        2500,        string()};
    mapInfo[5] = MapInfoS{5,
                          L"单路（无环）沼泽迷宫",
                          L"Single-Path Swamp Maze",
                          L"LocalGen",
                          50,
                          50,
                          2500,
                          ...
                          2500,
                          string()};
    mapInfo[6] = MapInfoS{6,
                          L"多环沼泽迷宫",
                          L"Multi-Ring Swamp Maze",
                          L"LocalGen",
                          50,
                          50,
                          2500,
                          ...
                          string()};
    mapInfo[7] = MapInfoS{
        7, L"全塔", L"Full Tower/City", L"LocalGen", 50, 50, 2500, 0, 2500, 0,
        0, string()};
    mapInfo[8] =
        MapInfoS{8, L"大平原", L"Great Plains", L"LocalGen", 50, 50, 2500, 0, 0,
                 0, 2500,      string()};
