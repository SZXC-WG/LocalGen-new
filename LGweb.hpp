...
void LGserver::zipSendBuf() {
    ...
    for (i = 1; i <= mapH; i++)
        for (j = 1; j <= mapW; j++) {
            ...
            for (k2 = 1; k2 <= 8; k2++) sendBuf[p++] = PMod(k1) + CHAR_AD;
        }
    sendBuf[p] = '\0';
}
...
void LGserver::procMessage(int sockID) {
    if (recvBuf[0] == 43) {
        ...
        if (recvBuf[1] != CHAR_AD)
            LGgame::analyzeMove(sockID, recvBuf[1] - CHAR_AD,
                                LGgame::playerCoo[sockID]);
        else
            LGgame::playerCoo[sockID] = {recvBuf[2] - CHAR_AD,
                                         recvBuf[3] - CHAR_AD};
    }
}
...
void LGserver::sockBroadcast() {
    ...
    for (int i = 1; i < totSock; i++)
        if (sockCon[i]) send(serverSocket[i], sendBuf, sizeof(sendBuf), 0);
...
int LGserver::GAME() {
    ...
    startBox
        .size(200 * LGGraphics::windowData.zoomX,
              100 * LGGraphics::windowData.zoomY)
        .move(400 * LGGraphics::windowData.zoomX,
              350 * LGGraphics::windowData.zoomY)
    ...
    for (; is_run() && lisCon; delay_fps(60)) {
        ...
        cleardevice();
        startBox.display();
        xyprintf(800 * LGGraphics::windowData.zoomX,
                 400 * LGGraphics::windowData.zoomY, "Player Number : %d",
                 plCnt);
        sockCollect();
        ...
        while (mousemsg()) {
            ...
            if (msg.x >= 400 * LGGraphics::windowData.zoomX &&
                msg.x <= 600 * LGGraphics::windowData.zoomY &&
                msg.y >= 350 * LGGraphics::windowData.zoomY &&
                msg.y <= 450 * LGGraphics::windowData.zoomY) {
                startBox.status = 1;
    ...
    for (; is_run(); delay_fps(LGgame::gameSpeed)) {
        ...
        while (kbmsg()) {
            ...
            if (ch.key == 27) {
                MessageBoxW(getHWnd(), wstring(L"YOU QUIT THE GAME.").c_str(),
        ...
        for (int i = plCnt + 1; i <= plCnt + rbCnt; ++i) {
            ...
            switch (robotId[i]) {
                case 1 ... 100: {
                    moveS mv =
                        smartRandomBot::calcNextMove(i, LGgame::playerCoo[i]);
                ...
                case 201 ... 300: {
                    moveS mv =
                        xiaruizeBot::calcNextMove(i, LGgame::playerCoo[i]);
                    if (LGgame::checkMove(mv)) LGgame::inlineMove.push_back(mv);
                } break;
            }
        }
...

        {
            std::chrono::nanoseconds timePassed =
                ...
                LGgame::beginTime;
            int needFlushToTurn =
                ceil(timePassed.count() / 1000000000.0L * LGgame::gameSpeed);
            ...
            if (lackTurn < 0)
                ...
                while (lackTurn > 0) {
                    timePassed =
                        std::chrono::steady_clock::now().time_since_epoch() -
                        LGgame::beginTime;
                    needFlushToTurn = ceil(timePassed.count() / 1000000000.0L *
                                           LGgame::gameSpeed);
                ...
                turnlen = textwidth(
                    ("Turn " + to_string(LGgame::curTurn) + ".").c_str());
                rspeedlen =
                    textwidth(("Real Speed: " +
                               to_string(LGgame::curTurn * 1.0L /
                                         (timePassed.count() / 1000000000.0L)))
                                  .c_str());
                setfillcolor(RED);
                setfillcolor(GREEN);
                ...
                xyprintf(screenszr - rspeedlen / 2 - 5, 0, "Real Speed: %Lf",
                         LGgame::curTurn * 1.0L /
                             (timePassed.count() / 1000000000.0L));
    ...
    for (int i = 1; i < totSock; i++) closesocket(serverSocket[i]);
...
void LGclient::dezipRecvBuf() {
    ...
    for (i = 1; i <= mapH; i++)
        for (j = 1; j <= mapW; j++) {
            ...
            for (k = 7; k >= 0; k--)
                gameMap[i][j].army =
                    (gameMap[i][j].army << 6) + recvBuf[p + k] - CHAR_AD;
...
void LGclient::sockConnect() {
    ...
    int res =
        connect(clientSocket, (LPSOCKADDR)&connectAddr, sizeof(connectAddr));
    u_long iMode = 1;
    ioctlsocket(clientSocket, FIONBIO, &iMode);
...
void LGclient::procMessage() {
    if (recvBuf[0] == 42)
        ...
        if (recvBuf[1] == CHAR_AD) {
            ...
            playerNumber = recvBuf[1] - CHAR_AD;
    } else
        dezipRecvBuf();
}
...
int LGclient::GAME() {
    ...
    IPfin
        .size(200 * LGGraphics::windowData.zoomX,
              100 * LGGraphics::windowData.zoomY)
        .move(100 * LGGraphics::windowData.zoomX,
              350 * LGGraphics::windowData.zoomY)
        .textalign(CENTER_TEXT, CENTER_TEXT)
        .fontname(LGset::mainFontName.c_str())
        .fontsize(50 * LGGraphics::windowData.zoomY, 0)
    ...
    IPbox.create(true);
    IPbox.move(800 * LGGraphics::windowData.zoomX,
               350 * LGGraphics::windowData.zoomY);
    IPbox.size(400 * LGGraphics::windowData.zoomX,
               100 * LGGraphics::windowData.zoomY);
    IPbox.setbgcolor(WHITE);
    IPbox.setcolor(LGGraphics::mainColor);
    IPbox.setfont(50 * LGGraphics::windowData.zoomY, 0,
                  LGset::mainFontName.c_str());
    IPbox.visible(true);
    ...
    for (; is_run(); delay_fps(60)) {
        ...
        while (mousemsg()) {
            ...
            if (msg.x >= 100 * LGGraphics::windowData.zoomX &&
                msg.x <= 300 * LGGraphics::windowData.zoomY &&
                msg.y >= 350 * LGGraphics::windowData.zoomY &&
                msg.y <= 450 * LGGraphics::windowData.zoomY) {
                IPfin.status = 1;
    ...
    for (; is_run() && lisCon; delay_fps(60)) {
        cleardevice();
        quitBox.display();
        xyprintf(1000 * LGGraphics::windowData.zoomX,
                 400 * LGGraphics::windowData.zoomY, "Player Number : %d",
                 playerNumber);
        sockCollect();
    ...
    for (; is_run(); delay_fps(std::min(LGgame::gameSpeed + 0.5, 120.5))) {
                ...
                if (msg.is_down()) {
                    ...
                    if (!moved && now - prsttm < 300ms) {
                        if (msg.x >= LGGraphics::windowData.maplocX &&
                            ...
                            msg.y <= LGGraphics::windowData.maplocY +
                                         blockHeight * mapH) {
                            movLin = (msg.y - LGGraphics::windowData.maplocY +
                                      blockHeight - 1) /
                                     blockHeight;
                            movCol = (msg.x - LGGraphics::windowData.maplocX +
                                      blockWidth - 1) /
                                     blockWidth;
                            movement.clear();
        ...
        while (kbmsg()) {
            ...
            switch (ch.key) {
                ...
                case 27:       {
                    MessageBoxW(getHWnd(),
                                wstring(L"YOU QUIT THE GAME.").c_str(), L"EXIT",
                ...
                case int('\b'): {
                    if (!LGgame::isAlive[playerNumber]) break;
        ...
        while (!movement.empty()) movement.pop_front();
