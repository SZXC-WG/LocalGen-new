...
int calcNextMove(int id, playerCoord coo) {
    ...
    using ll = long long;
    static int swampDir[20] = {3, 3, 3, 3, 3, 3, 3, 3, 3, 3,
                               3, 3, 3, 3, 3, 3, 3, 3, 3, 3};
    ...
    for (int i = 1; i <= 4; i++) {
        ...
        if (gameMap[tx][ty].type == 2 || tx < 1 || tx > mapH || ty < 1 ||
            ty > mapW)
            continue;
        p[++cnt] = {i, gameMap[tx][ty].player, gameMap[tx][ty].army,
                    gameMap[tx][ty].army, gameMap[tx][ty].type};
    ...
    //	printf(": ");
    //	for(int i=1; i<=cnt; ++i) printf("{%d %d %lld %lld %d}
    //",p[i].to,p[i].team,p[i].army,p[i].del,p[i].type); 	fflush(stdout);
    //_getch();
    for (int i = 1; i <= cnt; i++) {
