...
int getCnt(coordS player, int i) {
    ...
    des.x = player.x + dx[i];
    des.y = player.y + dy[i];
    if (gameMap[des.x][des.y].type == 2) return -100;
    if (des.x < 1 || des.x > mapH || des.y < 1 || des.y > mapW) return 500000;
    ...
    if (des.x == previousPos[id].x && des.y == previousPos[id].y)
        cnt += turnCount[id] * 10;
    if (des.teamOnIt != id && des.teamOnIt != 0) cnt--;
    if (des.type == 0) cnt--;
    if (des.type == 1) cnt += 2;
    if (des.teamOnIt == 0) cnt--;
    if (des.teamOnIt == id && des.Army >= 2000) cnt--;
    return cnt;
}
...
int calcNextMove(int ind, coordS player) {
    if (isEx) {
        ...
        if (gameMap[player.x][player.y].army == 0 ||
            gameMap[player.x][player.y].player != id) {
        ...
        for (int j = 1; j <= 4; j++) {
            ...
            des.x = player.x + dx[i];
            des.y = player.y + dy[i];
            if (gameMap[des.x][des.y].type == 2) continue;
            ...
            if (gameMap[des.x][des.y].player != id &&
                gameMap[des.x][des.y].type == 3) {
                previousPos[id] = player;
                return i;
            }
            if (des.type == 4 && des.Army <= nowPos.Army &&
                des.teamOnIt != id) {
                previousPos[id] = player;
                return i;
            }
    ...
    for (int j = 1; j <= 4; j++) {
        ...
        des.y = player.y + dy[i];
        if (gameMap[des.x][des.y].type == 2) continue;
        if (des.x < 1 || des.x > mapH || des.y < 1 || des.y > mapW) continue;
        okDir = i;
        des.type = gameMap[des.x][des.y].type;
        des.Army = gameMap[des.x][des.y].army;
        des.teamOnIt = gameMap[des.x][des.y].player;
    ...
    while (timeToTry--) {
        ...
        //                continue;
        //            if (des.x < 1 || des.x > mapH || des.y < 1 || des.y >
        //            mapW)
        //                continue;
        //            des.type = gameMap[des.x][des.y].type;
        //            des.Army = gameMap[des.x][des.y].army;
        ...
        //            gameMap[des.x][des.y].type == 3)
        //                return i;
        //            if (des.type == 4 && des.Army <=
        //            gameMap[player.x][player.y].army && des.teamOnIt == 0)
        //                return i;
        //            int cnt = 4;
        if (mtrd() % cnt[i] == 0) {
            previousPos[id] = player;
