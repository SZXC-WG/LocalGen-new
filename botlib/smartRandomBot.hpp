...
moveS calcNextMove(int id, coordS coo) {
    ...
    for (int i = 1; i <= 4; ++i) {
        ...
        if (nx < 1 || nx > mapH || ny < 1 || ny > mapW ||
            unpassable(gameMap[nx][ny].type))
            continue;
        p[++pl] = {gameMap[nx][ny].type, gameMap[nx][ny].player,
                   gameMap[nx][ny].army, i,
    ...
    return moveS{id, true, coo,
                 coordS{coo.x + dx[p[1].dir], coo.y + dy[p[1].dir]}};
}
}  // namespace smartRandomBot
