...
namespace xiaruizeBot {
    ...
    int dfs(int id, coordS coord) {
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            if (ToX < 1 || ToX > mapH || ToY < 1 || ToY > mapW) continue;
            if (unpassable(gameMap[ToX][ToY].type)) continue;
            if (gameMap[ToX][ToY].type == 3 && gameMap[ToX][ToY].player != id) {
                ...
                if (gameMap[ToX][ToY].army < gameMap[coord.x][coord.y].army) {
                    if (!operation[gameMap[ToX][ToY].player].empty())
                        operation[id].insert(
                            operation[id].end(),
                            operation[gameMap[ToX][ToY].player].begin(),
                            operation[gameMap[ToX][ToY].player].end());
                }
                return x;
            }
        }
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            if (ToX < 1 || ToX > mapH || ToY < 1 || ToY > mapW) continue;
            if (unpassable(gameMap[ToX][ToY].type)) continue;
            if (gameMap[ToX][ToY].type == 4 &&
                gameMap[ToX][ToY].army <= gameMap[coord.x][coord.y].army &&
                gameMap[ToX][ToY].player != id) {
                operation[id].push_back(x);
                previousPos[id] = coord;
                return x;
            }
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            if (ToX < 1 || ToX > mapH || ToY < 1 || ToY > mapW) continue;
            if (unpassable(gameMap[ToX][ToY].type)) continue;
            if (previousPos[id].x == ToX && previousPos[id].y == ToY) continue;
            operation[id].push_back(x);
            previousPos[id] = coord;
            return x;
        }
    ...
    moveS calcNextMove(int id, coordS coord) {
        if (gameMap[coord.x][coord.y].army == 0 ||
            gameMap[coord.x][coord.y].player != id) {
            memset(vis[id], 0, sizeof(vis[id]));
            backCountCnt[id]         = 1;
            otherRobotProtection[id] = std::max(
                0, std::min((int)operation[id].size() - 10, (int)mtrd() % 10));
        ...
        if (sendArmyProcess[id]) {
            ...
            if (otherRobotProtection[id]) {
                ...
                return moveS {
                    id, false, coord,
                    coordS {
                            coord.x + dx[operation[id][sendArmyProcess[id] - 2]],
        ...
        if (returnValue != -1) {
            ...
            return moveS {
                id, true, coord,
                coordS { coord.x + dx[returnValue], coord.y + dy[returnValue] }
            };
        } else {
            int res;
