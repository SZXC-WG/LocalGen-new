...
moveS calcNextMove(int ind, coordS player) {
    ...
    for (int j = 1; j <= 4; j++) {
        ...
        des.x = player.x + dx[i];
        des.y = player.y + dy[i];
        if (unpassable(gameMap[des.x][des.y].type)) continue;
        ...
        if (gameMap[des.x][des.y].player != id &&
            ...
            return moveS{id, true, player,
                         coordS{player.x + dx[i], player.y + dy[i]}};
        }
    ...
    return moveS{id, true, player,
                 coordS{player.x + dx[okDir], player.y + dy[okDir]}};
}
}  // namespace xrzBot
