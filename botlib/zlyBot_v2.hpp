...
namespace zlyBot_v2 {
	...
	inline void initBot(int id) {
		blockValueWeight[id] = {100*(30-LGset::plainRate[LGset::gameMode]+1),-INF,-INF,500,3000,3000};
		for(int i=1; i<=LGgame::playerCnt; ++i) seenGeneral[id][i] = coordS(-1,-1);
		memset(blockTypeRem[id],-1,sizeof(blockTypeRem[id]));
		for(int i=1; i<=mapH; ++i) {
			for(int j=1; j<=mapW; ++j) {
				blockArmyRem[id][i][j]=0;
				if(getType(id,i,j)==5) blockArmyRem[id][i][j]=40;
				if(getType(id,i,j)==2) blockArmyRem[id][i][j]=INF;
			}
		}
	...
	inline void calcData(int id, coordS coo) {
		...
		std::queue<std::pair<coordS,ll>> q;
		q.push({coo,0});
		while(!q.empty()) {
			coordS cur = q.front().first;
			ll curDist = q.front().second;
			q.pop();
			for(int i=0; i<4; ++i) {
				...
				ll newDist = curDist+(gmp(next)==id?-gma(next):(int)log(blockArmyRem[id][next.x][next.y]+1));
				dist[id][next.x][next.y] = newDist;
				// if(getType(id,next.x,next.y)==5) continue;
				q.push({next,curDist+1+newDist});
			}
		}
		for(int i=1; i<=mapH; ++i) {
			for(int j=1; j<=mapW; ++j) {
				if(gameMap[i][j].player==id) blockValue[id][i][j] = -INF;
				else blockValue[id][i][j] = blockValueWeight[id][gameMap[i][j].type] - dist[id][i][j]*10;
			}
		}
	}
	...
	inline void findRoute(int id, coordS coo, coordS desti) {
		int cnt=0;
		for(int i=1; i<=mapH; ++i) for(int j=1; j<=mapW; ++j) if(gameMap[i][j].player==id) ++cnt;
		cnt=sqrt(cnt)+1;
	...
	moveS calcNextMove(int id, coordS coo) {
		for(int i=1; i<=mapH; ++i)
			for(int j=1; j<=mapW; ++j) {
				if(isVisible(i,j,1<<id)) {
					if(gmt(i,j)!=2) blockArmyRem[id][i][j] = gma(i,j);
					else blockArmyRem[id][i][j]=INF;
				}
		...
		int goalGeneralId = -1; ll goalGeneralArmy = INF;
		...
		if(botMode[id] == BOT_MODE_ATTACK) {
			...
			struct node { coordS coo; ll value; };
			vector<node> v;
			for(int i=1; i<=mapH; ++i) for(int j=1; j<=mapW; ++j) v.push_back(node{coordS(i,j),blockValue[id][i][j]});
