...
namespace zlyBot_v2 {
	...
	moveS calcNextMove(int playerId, coordS currentPos) {
				...
				if(isVisible(row, col, 1 << playerId) &&
				   ...
				   gameMap[row][col].type == 3)
					seenGenerals[playerId][gameMap[row][col].player] =
					    coordS(row, col);
#ifdef DEBUG_ZLY_V2
		db.open("player_"s + to_string(playerId) + "_debug.txt"s,
		        std::ios::app);
		db << std::endl;
#endif
		...
		for(int i = 1; i <= LGgame::playerCnt; ++i) {
			if(seenGenerals[playerId][i] != coordS(-1, -1) &&
			   LGgame::isAlive[i]) {
		...
		if(botModes[playerId] == BOT_MODE_ATTACK) {
			...
			moveS ret = moveS{ playerId, true, currentPos,
			...
			for(int row = 1; row <= mapH; ++row)
				for(int col = 1; col <= mapW; ++col)
					nodes.push_back(node{ coordS(row, col),
					                      blockValue[playerId][row][col] });
			sort(nodes.begin(), nodes.end(), [](const node& a, const node& b) {
				return a.value > b.value;
			});
#ifdef DEBUG_ZLY_V2
			db << "Desti: " << nodes[0].pos.x << "," << nodes[0].pos.y
			   << std::endl;
