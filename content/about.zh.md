---
title: "项目简介"
description: "了解 LocalGen v6 可以做什么、包含哪些工具，以及哪些功能仍在开发中。"
...
当前开发版本为 **`6.0.0-dev`**。带有 `-dev` 的版本仍在持续开发，部分菜单功能暂不可用。
...
- v6 目前还不能打开 `.lgr` / `.lgra` 回放，也不会在重启后保留应用设置。

## Bot 是编译代码，不是可下载模型

v6 使用随应用提供、通过 `BotFactory` 注册的 C++ Bot，暂不支持可下载模型、插件或外部 Bot 进程。
...
- [访问项目仓库](https://github.com/SZXC-WG/LocalGen-new)
