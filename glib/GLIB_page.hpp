...
inline namespace page {
	...
	inline void itemS::downLoc() {
		switch(iType) {
			case ITEM_SUBPAGE: { info.subPage->move(locX,locY); } break;
			// case ITEM_CONDTEXT: { info.cdtnText->move(locX,locY); } break;
			case ITEM_RECTBUTTON: { info.rButton->move(locX,locY); } break;
			case ITEM_CIRCBUTTON: { info.cButton->move(locX,locY); } break;
			case ITEM_RECTCHKBOX: { info.rChkBox->move(locX,locY); } break;
			case ITEM_RECTCHKBOX_WITH_TEXT: { info.cBText->move(locX,locY); } break;
		}
	}
	...
	inline void itemS::work(_Ftn_t _ftn) {
		switch(iType) {
			...
			case ITEM_VARINTTEXT: { _ftn(*info.vIText); } break;
	...
	inline _Rtn_t itemS::workRet(_Ftn_t _ftn) {
		...
		switch(iType) {
			...
			case ITEM_VARINTTEXT: { ret = _ftn(*info.vIText); } break;
	...
	inline bool pageS::detect(mouse_msg _mouse) {
		...
		for(auto& it:content) {
			bool success = false;
			switch(it.iType) {
				case ITEM_SUBPAGE: { success = it.info.subPage->detect(_mouse); } break;
				case ITEM_RECTBUTTON: { success = it.info.rButton->detect(_mouse); } break;
				case ITEM_CIRCBUTTON: { success = it.info.cButton->detect(_mouse); } break;
				case ITEM_RECTCHKBOX: { success = it.info.rChkBox->detect(_mouse); } break;
				case ITEM_RECTCHKBOX_WITH_TEXT: { success = it.info.cBText->detect(_mouse); } break; // to do
				default: break; // nothing
			}
	...
	inline pageS& pageS::draw() {
		...
		for(auto& it:content) {
			switch(it.iType) {
				case ITEM_SUBPAGE: { it.info.subPage->display(pageImage); } break;
				case ITEM_VARINTTEXT: { it.info.vIText->print(it.locX,it.locY,pageImage); } break;
				case ITEM_LINETEXT: { it.info.lText->print(it.locX,it.locY,pageImage); } break;
				// case ITEM_CONDTEXT: { it.info.cdtnText->print(); } break;
				case ITEM_RECTBUTTON: { it.info.rButton->display(pageImage); } break;
				case ITEM_CIRCBUTTON: { it.info.cButton->display(pageImage); } break;
				case ITEM_RECTCHKBOX: { it.info.rChkBox->display(pageImage); } break;
				case ITEM_RECTCHKBOX_WITH_TEXT: { it.info.cBText->display(it.locX,it.locY,pageImage); } break;
			}
		}
	}
