   ...
   public:
    ...
    moveS calcNextMove(coordS coo) {
        ...
        if (!isValidPosition(coo.x, coo.y) ||
            gameMap[coo.x][coo.y].player != id ||
            gameMap[coo.x][coo.y].army < 2)
            coo = maxArmyPos(), prevTarget = coordS(-1, -1);
        else if (coo == prevTarget ||
                 (prevTarget.x != -1 &&
                  gameMap[prevTarget.x][prevTarget.y].player == id))
            prevTarget = coordS(-1, -1);
        ...
        for (int i = 1; i <= LGgame::playerCnt; ++i) {
            if (i != id && seenGeneral[i] != coordS(-1, -1) &&
                ...
                if (thatArmy < minArmy) minArmy = thatArmy, targetId = i;
            }
        }
        ...
        if (targetId != -1)
            ...
            if (dis(rnd) < 0.02) {
                // Exploration: find random plain with unknown type (might
                // be a general)
                ...
                if (!unknownPlains.empty()) {
                    std::uniform_int_distribution<int> randIndex(
            ...
            for (int i = 1; i <= mapH; ++i)
                for (int j = 1; j <= mapW; ++j)
                    if (gameMap[i][j].player != id && dist[i][j] < 500 &&
                        !unpassable(blockType[i][j])) {
                        ll blockValue = blockTypeValue[blockType[i][j]] +
                                        eval[i][j] / 5 -
                                        (dist[i][j] + army[i][j] / 2);
                        if (LGset::gameMode == 0)
                            blockValue -=
                                approxDist({i, j}, seenGeneral[id]) * 5ll;
                        if (blockValue > maxBlockValue)
                            maxBlockValue = blockValue, targetPos = {i, j};
                    }
            int x = prevTarget.x, y = prevTarget.y;
            if (x != -1 && gameMap[x][y].player != id &&
                !unpassable(blockType[x][y])) {
                ll prevBlockValue = blockTypeValue[blockType[x][y]] +
                                    eval[x][y] / 5 -
                                    (dist[x][y] + army[x][y] / 2);
                if (LGset::gameMode == 0)
