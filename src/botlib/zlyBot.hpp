...
inline int getType(int id, int x, int y) {
    ...
    else if (unpassable(gameMap[x][y].type) || gameMap[x][y].type == 4)
        return 8;
...
inline void initBot(int id) {
    blockValueWeight[id] = {30 - LGset::plainRate[LGset::gameMode] + 1,
                            -1500,
                            -INF,
                            5,
                            30,
    ...
    for (int i = 1; i <= mapH; ++i) {
        for (int j = 1; j <= mapW; ++j) {
            if (gameMap[i][j].player == id)
                blockValue[id][i][j] = -INF;
            else
                blockValue[id][i][j] =
                    blockValueWeight[id][getType(id, i, j)] - dist[id][i][j];
        }
    }
}
...
inline void findRoute(int id, coordS coo, coordS desti) {
    ...
    pr.push_back(vector<vector<coordS>>(
        mapH + 1, vector<coordS>(mapW + 1, coordS(-1, -1))));
        ...
        for (int x = 1; x <= mapH; ++x) {
            for (int y = 1; y <= mapW; ++y) {
                ...
                for (int j = 0; j < 4; ++j) {
                    ...
                    // if(nv>=-INF/10)
                    // 	printf("i(%d) cnt(%d) fx(%d) fy(%d) nx(%d) ny(%d)
                    // dp(%lld) pr(%d,%d) nv(%lld)\n",
...
moveS calcNextMove(int id, coordS coo) {
    for (int i = 1; i <= mapH; ++i)
        for (int j = 1; j <= mapW; ++j)
            if (isVisible(i, j, 1 << id) && gameMap[i][j].player != id &&
                gameMap[i][j].type == 3)
                seenGeneral[id][gameMap[i][j].player] = coordS(i, j);
    // std::ofstream
    // db("player_"s+to_string(id)+"_debug.txt"s,std::ios::app);
    if (gameMap[coo.x][coo.y].player != id || gameMap[coo.x][coo.y].army == 0) {
        ...
        // db << "Turn " << LGgame::curTurn << " from (" << coo.x << "," <<
        // coo.y << ") to (" << mxCoo.x << "," << mxCoo.y << ")" <<
        // std::endl; db.close(); return moveS{ id, false, coo, mxCoo };
        coo = mxCoo;
    }
    ...
    if (botMode[id] == BOT_MODE_ATTACK) {
        // db << "Desti: " << seenGeneral[id][goalGeneralId].x << "," <<
        // seenGeneral[id][goalGeneralId].y << std::endl;
        findRoute(id, coo, seenGeneral[id][goalGeneralId]);
        moveS ret = moveS{id, true, coo, stackedMove[id].front()};
        // db << "stackedMove size: " << stackedMove[id].size() <<
        // std::endl;
        stackedMove[id].pop_front();
        ...
        for (int i = 1; i <= mapH; ++i)
            for (int j = 1; j <= mapW; ++j)
                v.push_back(node{coordS(i, j), blockValue[id][i][j]});
        sort(v.begin(), v.end(),
